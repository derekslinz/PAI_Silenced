...
<div class="cover">
  ...
  <div class="company-name">{YOUR_BUSINESS_NAME} CONSULTING</div>
  <div class="footer-note">CONFIDENTIAL — For Authorized Recipients Only</div>
</div>
```

**Logo source:** Your logo should be at `report/public/your-logo.png`. Use `file://` protocol for Playwright rendering.

**Footer branding:** The Playwright `footerTemplate` must include "{YOUR_BUSINESS_NAME} Consulting" centered between CONFIDENTIAL (left) and page number (right).
...
The Quorum Cyber strategic assessment (`/path/to/reference/generate-pdf.mjs`) is the reference implementation of this workflow. Key stats:
