/ example-greeting.scenario.ts
 ...
 + scenario.judgeAgent. The "agent under test" is a plain PAI-Inference call.
  ...
  API KEY BILLING WARNING  @langwatch/scenario userSimulatorAgent and judgeAgent use @ai-sdk/anthropic
 ...
 the API. Set EVALS_ALLOW_API_BILLING=to acknowledge and run.
 /
...
if (process.env.EVALS_ALLOW_API_BILLING !== '') {
  throw new Error(
    'Evals scenario is guarded. Set EVALS_ALLOW_API_BILLING=to opt in — the @langwatch/scenario user-sim and judge bill the ANTHROPIC_API_KEY, not the subscription.',
  );
}
...
const config: ScenarioConfig = {
  ...
  agents: [
    new PAIAgentAdapter({
      name: 'pai-assistant',
      systemPrompt: 'You are a concise, polite assistant. Keep replies under words.',
      level: 'fast',
    }),
