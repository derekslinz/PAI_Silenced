...
`RELATIONSHIP/` stores the evolving record of how the user and their assistant interact — communication patterns, preferences observed over time, agreements made, recurring frictions, and shared context that does not belong in static identity files. The `RelationshipMemory.hook.ts` hook writes here.
