...
Browser-verify all web output. ALL web-based output must be verified through the **agent-browser** BEFORE showing to {{PRINCIPAL_NAME}}. agent-browser is the ONLY sanctioned browser automation in PAI — real Chrome, no CDP detection, real login sessions, accurate rendering. agent-browser is deprecated for verification and misses rendering issues that real Chrome catches. Playwright is BANNED — if you are tempted to use it, fix agent-browser instead. "curl returns 200" is not verification. A screenshot from agent-browser is not verification. You must verify with agent-browser. **Every time you create, fix, deploy, or claim anything works on the web — verify with agent-browser. No exceptions.**
