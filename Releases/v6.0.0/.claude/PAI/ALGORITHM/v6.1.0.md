...
. Multi-project / cross-cutting — ≥ distinct project names from PROJECTS.md, OR references to `MEMORY/`, `KNOWLEDGE/` → floor: E
...
Before you emit the closing of an Algorithm run, check yourself: is the last thing on screen the `  SUMMARY  /` block, with ` ITERATION`, ` CONTENT`, ` STORY`, ` {{ASSISTANT}}` fields?

Invariant: Phase / = SUMMARY block. The response ends at ` {{ASSISTANT}}: …`. Nothing follows.
