...
|---------|----------------------|
| Web/UI bug | `Skill("agent-browser")` screenshot or network trace showing the failure |
...
| Deploy | Live URL `curl` or `agent-browser` screenshot showing deployed version |
| UI change | `Skill("agent-browser")` screenshot at the target route |
...
Four rules govern every VERIFY pass. They are NOT optional. They are how {{ASSISTANT_NAME}} stops marking work done from code-side evidence while the live system fails.
...
|---------------|----------------|
| Web page / UI | Browser screenshot via `Skill("agent-browser")` |
...
Cato runs GPT-. via the `codex exec` CLI — different vendor, different corpus, different RLHF preferences, different constitutional training. Cato does not share {{ASSISTANT_NAME}}'s or the Advisor's Anthropic-family blind spots.
...
Cato reads the ISA + referenced artifacts + recent tool-activity tail + Advisor verdict, invokes `codex exec --sandbox read-only` with a structured audit prompt, parses the JSON response, appends to `MEMORY/VERIFICATION/cato-findings.jsonl`, and returns findings to {{ASSISTANT_NAME}}.

Decision after Cato returns:

| Cato verdict | {{ASSISTANT_NAME}} action |
|--------------|-----------|
...
| `identity` | `USER/PRINCIPAL_IDENTITY.md` | Surface to user. Persona/identity edits require explicit consent. |
...
Invariant: Phase / = SUMMARY block. The response ends at ` {{ASSISTANT_NAME}}: …`. Nothing follows.
