...
Typical Cost column (renamed from "Tier Fit" in Algorithm v..): the lowest effort tier at which this capability typically fits the budget. Pure information — not a restriction. The model decides per-task whether the capability is worth its cost given the tier time budget. At E/E, capabilities marked E+ usually blow the budget; at E anything fits.

| Capability | Phases | Trigger Signal | Invoke | Typical Cost |
|------------|--------|----------------|--------|----------|
| IterativeDepth | OBSERVE | Default at Extended+ when time budget allows deeper understanding; any important task where exploring the full problem space before ISC improves outcome; understanding what's actually being asked vs what was literally said; exploring different approach angles before committing; ambiguous scope, multi-faceted problems, hidden assumptions | `Skill("IterativeDepth")` | E+ |
...
| Council | THINK, PLAN | Multi-perspective decision, trade-offs, controversial direction | `Skill("Council")` | E+ |
| RedTeam | THINK, VERIFY | Strategy validation, stress-test plan, attack assumptions | `Skill("RedTeam")` | E+ |
| Science | THINK→EXECUTE | Debugging hypothesis, systematic investigation, optimization | `Skill("Science")` | E+ |
| BeCreative | OBSERVE, BUILD | Novel approaches needed, brainstorming, divergent thinking | `Skill("BeCreative")` | E+ |
| Ideate | BUILD, EXECUTE | Multi-cycle idea generation, evolutionary ideation | `Skill("Ideate")` | E+ |
| BitterPillEngineering | VERIFY | Audit for over-engineering, dead weight, fragile scaffolding | `Skill("BitterPillEngineering")` | E+ |
| Evals | VERIFY | Objective measurement, prompt comparison, quality scoring | `Skill("Evals")` | E+ |
| WorldThreatModel | THINK | Long-term strategy stress-test, future-proofing | `Skill("WorldThreatModel")` | E |
| Fabric patterns | any | Targeted transform via a specific Fabric pattern (extract_wisdom, summarize, etc.) | `Skill("Fabric")` | E+ |
| ContextSearch | OBSERVE | Prior PAI work, session recovery, cold-start | `Skill("ContextSearch")` | E+ |
| ISA Skill | OBSERVE, PLAN, EXECUTE, VERIFY, LEARN | MANDATORY at E+ for ISA scaffolding (`Skill("ISA", "scaffold from prompt at tier T")`), tier completeness checks (`Skill("ISA", "check completeness")`), ephemeral feature extraction at PLAN, canonical Decisions/Changelog/Verification entries via Append at any phase, and Reconcile after ephemeral feature work at LEARN. E may inline-write the minimal Goal+Criteria ISA to preserve <s budget. The skill owns the canonical twelve-section template and refuses to write partial Deutsch C/R/L Changelog entries. | `Skill("ISA", "<verb> <args>")` | E+ |
...
|------------|------|--------|
| Forge (code producer) | MANDATORY at E/E/E for any coding task (implement, refactor, debug, build). Also invoke whenever {{PRINCIPAL_NAME}} names "Forge" at any tier. OpenAI-family coder — GPT-. via `codex exec` at `model_reasoning_effort=high`. Specialization: quality + completeness. Distinct from Engineer (Claude-family) and Cato (auditor, read-only). DO NOT invoke at E/E — cost/latency prohibitive. | `Agent(subagent_type="Forge", prompt="...")` |
| Anvil (Kimi K. code producer) | Sibling to Forge, Moonshot-family. Runs `kimi-k.` (reasoning model, Moonshot enforces temperature=) via `PAI/TOOLS/AnvilProgress.ts` with Moonshot's K context. Pick Anvil over Forge when the task benefits from whole-project context breadth — cross-file refactors, architecture-fitting changes, long-range reasoning. Always invoke when {{PRINCIPAL_NAME}} names "Anvil" (name-match overrides tier gate). At E/E/E, Forge remains the default producer; Anvil is chosen instead of or in parallel with Forge. Skip at E/E unless {{PRINCIPAL_NAME}} named him. | `Agent(subagent_type="Anvil", prompt="...")` |
| /simplify | After code changes | `Skill("simplify")` |
| /batch | + files with similar changes | `Skill("batch", "instruction")` |
...
| /codex:adversarial-review | Challenge design decisions, question approach and tradeoffs | `Skill("codex:adversarial-review")` |
 ...
 Forge auto-include binding (E-E coding tasks)
...
What this gate prevents: E+ coding work silently routed through Claude-family only, repeating the same-family blind spot pattern that Cato addresses on the review side.
 ...
 Anvil invocation binding (E-E long-context coding tasks)
...
| Capability | When | Invoke |
|------------|------|--------|
| Agent Teams | DEFAULT for parallel work. + agents on related work, task dependencies, coordination needed. Teammates persist, self-claim tasks, message peers. | `TeamCreate` + `Agent` with `team_name` |
...
| Managed Agents | Unattended/overnight work. Hours-long tasks, survive disconnects, sandboxed cloud execution, CI triggers. $./session-hour + tokens. | `Skill("claude-api")` to build workflows |
| Delegation | + independent workstreams (routes to above) | `Skill("Delegation")` |
...
| Observer Team | ONLY when time is not a constraint AND auditability is the primary requirement. -agent read-only swarm watches `tool-activity.jsonl` (ground-truth audit log), votes continue/halt/escalate every s. Deliberate speed-for-safety trade — not for interactive work. Fit: overnight autonomous runs, production deploys needing post-hoc review, credential rotation, security-hook edits. | `Skill("Agents")` → `SPAWNOBSERVERS` workflow |
...
|----------|-----------|--------|--------|
| . DEFAULT | "parallel work", "agents", "team", "swarm", or Algorithm selects delegation | Agent Teams — persistent teammates, shared task list, peer messaging | `TeamCreate` + `Agent` with `team_name` |
| . EXPLICIT | "custom agents", "spin up custom agents" | Custom Agents — unique personalities, voices, trait composition | `Skill("Agents")` → ComposeAgent |
| . UNATTENDED | "run overnight", "long-running", "CI", or task exceeds session lifetime | Managed Agents — durable cloud sessions, sandboxed, vault credentials | `Skill("claude-api")` to build |
| . INTERNAL | (Algorithm internal routing, user names a type) | Built-in types (Designer, Architect, Engineer, Explore, etc.) | `Agent(subagent_type="...")` |
...
Codex commands run GPT-.-Codex as a second model for review or delegation. Management commands:
