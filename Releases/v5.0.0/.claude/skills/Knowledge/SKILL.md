---
name: Knowledge
description: "Manage the PAI Knowledge Archive — a curated, typed graph of notes across four entity domains: People, Companies, Ideas, and Research. Operations: search (-pass: lexical + frontmatter + wikilink), add (creates note with mandatory typed cross-links), harvest (KnowledgeHarvester pulls from PAI sources), develop (surfaces seedling notes for enrichment), ingest (fetch URL or file, create primary note, ripple updates to related notes), contradictions (find conflicting claims via tag-overlap pairs), graph (stats or -hop traversal via KnowledgeGraph.ts), retrieve (BM-lite compressed context via MemoryRetriever.ts), mine (SessionHarvester extracts memory candidates from recent conversations). Every note ships with typed related: frontmatter links (relationship types: supports, contradicts, extends, part-of, instance-of, caused-by, preceded-by, related). USE WHEN knowledge, knowledge base, search knowledge, what do we know about, archive, harvest, knowledge status, develop note, add to knowledge, ingest, contradictions, knowledge graph, graph, retrieve, mine conversations. NOT FOR session/ISA context recovery (use ContextSearch)."
...
Step — Lexical search:```bash
rg -i "$ARGUMENTS" ~/.claude/PAI/MEMORY/KNOWLEDGE/ --type md -l
```

Step — Frontmatter search (tags and titles):```bash
rg -i "title:.$ARGUMENTS|tags:.$ARGUMENTS" ~/.claude/PAI/MEMORY/KNOWLEDGE/ --type md -l
```

Step — Wikilink search:```bash
rg "\[\[.$ARGUMENTS.\]\]" ~/.claude/PAI/MEMORY/KNOWLEDGE/ --type md -l
```

Deduplicate results across all three. For each match, read the first lines of frontmatter to show title, domain, status, tags.
...
. MANDATORY: Find -related notes first.Before writing the new note, grep existing Knowledge for related entities by topic/tags/name. This becomes the `related:` frontmatter array. No Knowledge note ships without typed links. See Canonical Linking Requirement below.
. Create the note with proper frontmatter from `_schema.md` — schemas require: `title`, `type`, `tags` (min ), `created`, `updated`, `quality` (-), plus type-specific body sections.
. Write the file to `KNOWLEDGE/<Type>/<kebab-case-title>.md` — slug max chars
. Verify every slug in `related:` exists in the archive before saving
. Regenerate the type's MOC:
```bash
...
Every new Knowledge note must ship with typed cross-links.This is not optional. The architecture is defined in `MEMORY/KNOWLEDGE/Ideas/pai-knowledge-linking-architecture.md` (quality ) and the schema in `_schema.md`.

Every write must include:
. `related:` frontmatter array— -typed entries linking to other Knowledge entries (any domain: People, Companies, Ideas, Research)
. Body wikilinks— -`[[slug]]` references woven into the prose where natural (Implications, Evidence, or Context sections)

relationship types(pick the most accurate, prefer specific over generic):
...
How to find related notes before writing:```bash
By topic/keyword
...
Enforcement:- Writes that skip `related:` are incomplete and must be fixed before the skill/workflow returns success
...
Step — Find seedlings:```bash
rg "^status: seedling" ~/.claude/PAI/MEMORY/KNOWLEDGE/ --type md -l
```

Step — For each seedling:- Read the note
...
Step — Present the diff to the user for approval.
Step — If approved:- Write the updated note
...
Summarize the source in -sentences. Identify key entities, claims, and insights.

Step — Classify and create primary note
...
- Generate kebab-case slug from title (max chars)
- Write to `KNOWLEDGE/<Type>/<slug>.md` with proper frontmatter
- Include `source_url:` or `source_path:` in frontmatter
- MANDATORY: Include `related:` array with -typed links— the ripple pass (Step ) identifies these, and they must be baked into the frontmatter of the primary note at creation time, not added after

Step — Ripple pass (the key innovation)

Search for existing notes that relate to this new content:

```bash
Search by extracted tags
rg -i "TAG|TAG|TAG" ~/.claude/PAI/MEMORY/KNOWLEDGE/ --type md -l --glob '!_'

Search by key entities/concepts mentioned
rg -i "ENTITY|ENTITY" ~/.claude/PAI/MEMORY/KNOWLEDGE/ --type md -l --glob '!_'
```

For each related note found (up to ):
. Read the note
...
INGEST RIPPLE PLAN:
  ...
  PRIMARY related: frontmatter links (MANDATORY):
    → Ideas/existing-note-— type: extends
    → Ideas/existing-note-— type: supports
...
Step — Execute ripple updates

After the user approves (or you determine updates are low-risk cross-references):
- Primary note: ensure `related:` frontmatter array has -typed entries — this is mandatory, not optional
...
Step — Get contradiction candidates
...
This outputs pairs of notes with high tag overlap (+ shared tags), ranked by overlap count.

Step — Semantic review

For each pair (up to highest-overlap pairs):
. Read both notes
...
CONTRADICTION SCAN:
  ...
  SUPERSEDED:
    [[older-note]] (--): "X was true"
    [[newer-note]] (--): "X is no longer true because Y"
    Action: Update older note with correction
```

Step — Fix (with approval)
...
No argument — stats overview:```bash
...
With slug — traverse from a note:```bash
bun ~/.claude/PAI/TOOLS/KnowledgeGraph.ts traverse <slug> --hops ```

Show all notes connected within hops via tags, wikilinks, and typed relationships. Useful for exploring how knowledge connects across domains.

Related notes only:```bash
...
Compressed context retrieval over the Knowledge Archive using BM-lite scoring.

```bash
bun ~/.claude/PAI/TOOLS/MemoryRetriever.ts "<query>" --top ```
...
```bash
bun ~/.claude/PAI/TOOLS/SessionHarvester.ts --mine --recent ```
...
```bash
bun ~/.claude/PAI/TOOLS/SessionHarvester.ts --mine --recent --dry-run
```
...
- entity types now.People (human beings), Companies (organizations), Ideas (insights/theses/analyses), Research (multi-source investigations with methodology). If it doesn't fit one of these, it's not knowledge — it belongs in WORK/ or LEARNING/.
...
- Never delete notes without asking.Pruning is automatic (-day seedling expiry via harvester). Manual deletion requires the user's approval.
