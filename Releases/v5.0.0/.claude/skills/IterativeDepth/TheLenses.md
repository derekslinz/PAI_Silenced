...
Lens : LITERAL (Surface Requirements)
...
Lens : STAKEHOLDER (Who Else Cares?)
...
Lens : TEMPORAL (Past, Present, Future)
Question:"How does this change over time? What's the history? What happens in months?"
...
Example prompt variation:"What context created this request? What will change in -months that could invalidate this solution?"

---

Lens : EXPERIENTIAL (How Should It Feel?)
...
Lens : CONSTRAINT INVERSION (What If?)
...
Focus:Remove assumed constraints — what would we build with infinite time/resources? Then add extreme constraints — what if it had to work offline, in ms, with zero dependencies? Both directions reveal hidden assumptions.
...
Lens : ANALOGICAL (What Patterns Apply?)
...
Example prompt variation:"What are -analogous problems in other domains? What solutions worked there? What criteria would those solutions imply here?"

---

Lens : META (Is This the Right Question?)
...
|-----|-------------|------------|-------------|
| Instant| | Skip IterativeDepth entirely | s |
| Fast| | Literal + Failure | <s |
| Standard| | Literal + Stakeholder + Failure + Experiential | <min |
| Deep| | All lenses | <min |

At FastSLA, the two most commonly productive lenses (surface requirements + what goes wrong) run as brief internal thought exercises — not spawned agents.

At Standard, lenses run. These can be parallelized as pairs of background agents.

At Deep, all lenses run. These can be parallelized as pairs or individual agents for maximum coverage.

---

Lens Selection for Custom Depth

When invoked with a specific count (e.g., "do passes"), select lenses in order from Lens through Lens N. The ordering is designed so that earlier lenses are more universally applicable.
