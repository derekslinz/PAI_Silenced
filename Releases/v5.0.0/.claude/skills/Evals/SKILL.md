...
Comprehensive agent evaluation system based on Anthropic's "Demystifying Evals for AI Agents" (Jan ).
...
|------|-------------|---------|
| Capability| ~% | Stretch goals, measuring improvement potential |
| Regression| ~% | Quality gates, detecting backsliding |

Key Metrics

- pass@k: Probability of at least success in k trials (measures capability)
...
bun run ${CLAUDE_SKILL_DIR}/Tools/AlgorithmBridge.ts -s regression-core -r -u
```
...
|---|----------------------|--------|
| | Auth bypass fixed | eval:auth-security |
| | Tests all pass | eval:regression |
```
...
. Start with -real failures- Don't overthink, capture what actually broke
...
. Monitor saturation- Graduate to regression when hitting %+
...
- pass@k scoring requires multiple runs.A single run doesn't give statistical significance. Default to pass@minimum.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Evals","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
