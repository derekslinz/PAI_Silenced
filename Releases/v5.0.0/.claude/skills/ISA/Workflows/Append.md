...
Type — Changelog (the Deutsch C/R/L entry)
...
Step — Resolve target ISA and section
...
Step — Append to the section

Edit the ISA: append the entry to the end of the target section, preserving prior entries. Update frontmatter `updated: <ISO->`.

Step — Update progress (Verification only)
...
Step — Return the appended block

Output the exact text that was appended, plus the path. Caller can re-emit for confirmation.

Why this workflow exists

The Deutsch C/R/L Changelog format is the most opinionated piece of the v..doctrine and the easiest to dilute. Three failure modes if there's no canonical writer:

. Free-form prose creep:"we changed our minds about X" instead of the four-piece structure.
. Half-entries:`conjectured` + `criterion now` without the refutation evidence in between.
. Format drift:different projects evolve different conventions, breaking cross-project search and tooling.
