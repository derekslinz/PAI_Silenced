...
- After Scaffold at E(mandatory before BUILD per the tier completeness gate).
...
| max_questions | no | Default ; cap to keep interview tight |
...
Step — Ask questions one at a time
...
Step — Write back to ISA after each answer
...
Step — Final CheckCompleteness pass
...
- Conversational, not formal. No "Question of ."
- One question per turn. Never batch.
- Use the user's own words from earlier answers when framing later questions.
- If the user is rushed, drop to E/Erequired sections only and flag the rest as "fill in later."
