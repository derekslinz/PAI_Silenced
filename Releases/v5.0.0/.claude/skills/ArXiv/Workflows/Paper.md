...
- arXiv URL (`https://arxiv.org/abs/.` or `https://arxiv.org/pdf/.`)
- AlphaXiv URL (`https://alphaxiv.org/abs/.`)
- Paper ID (`.` or `.v`)

Steps

. Extract paper ID
Strip to just the numeric ID (e.g., `.`). Remove version suffixes for AlphaXiv lookup (it uses latest).

. Try AlphaXiv overview first
```bash
curl -s "https://alphaxiv.org/overview/PAPER_ID.md"
```

If : this is the primary source. It returns a structured markdown analysis optimized for language models.

. Fetch arXiv metadata regardless
```bash
...
. If AlphaXiv 'd, try full text
```bash
curl -s "https://alphaxiv.org/abs/PAPER_ID.md"
```

If this also s, work from the abstract only. Mention that the user can read the full PDF at `https://arxiv.org/pdf/PAPER_ID`.
...
Key Contributions
{-bullets on what's new/important}
