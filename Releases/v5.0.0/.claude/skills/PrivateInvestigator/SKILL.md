...
Running the WorkflowNameworkflow in the PrivateInvestigatorskill to ACTION...
```
...
Every investigation uses parallel research agents(types × each):

Agent Types:. ClaudeResearcher(agents) - People search aggregators, professional records, location intelligence, comprehensive identity, public records, education/alumni
. GeminiResearcher(agents) - Alternative identities, multi-perspective synthesis, historical context
. GrokResearcher(agents) - Social media deep search, contrarian analysis, real-time intelligence
. CodexResearcher(agents) - Username enumeration, Google dorking, technical profiles

Each agent executes sub-searches= parallel search threadsper investigation

Launch Pattern:All agents launch in a SINGLE message with multiple Task tool calls.
...
| Spokeo | Freemium | Social media aggregation (+ networks) |
...
Tier : Foundation Data- Full name (and variations/maiden names)
- Approximate age or date of birth
- Last known location
- Context (school, workplace, relationship)

Tier : Primary Research- People search aggregators
- Social media presence scan
- Google dorking

Tier : Deep Investigation- Public records searches
- Reverse lookups on discovered info
- Cross-platform correlation
- Associate/family network mapping

Tier : Verification- Multi-source confirmation
...
|-------|----------|--------|
| HIGH| + unique identifiers match across independent sources | Safe to contact |
| MEDIUM| identifiers match, timeline consistent | Verify before contact |
...
. Multiple Identifiers- Require + matching data points
...
Example : Finding an Old College Friend```
User: "Help me find my college roommate from , John Smith from Austin"
...
Related Documentation:- Complete workflow details in `Workflows/` directory
...
- parallel agents can hit rate limits on public records APIs.Stagger launches if services throttle.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"PrivateInvestigator","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
