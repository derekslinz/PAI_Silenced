...
When to Use:- User wants to find a specific person by name
...
Prerequisites:- Subject's name (ideally full name)
...
Step : Gather Initial Information

Description:Collect all available starting data from the user

Questions to Ask:. What is the person's full name? (including maiden name, nicknames, variations)
. What is their approximate age or date of birth?
. Where did you last know them to be located?
. How do you know this person? (school, work, family, etc.)
. When did you last have contact?
. Do you have any old phone numbers, emails, or addresses?
. Do you know any family members or mutual contacts?
...
Step : LAUNCH PARALLEL INVESTIGATION (+ Agents)

CRITICAL: Launch ALL agents in a SINGLE message with multiple Task tool calls
This is the main investigation step. Deploy + agents across categories simultaneously.

---

CATEGORY : People Search Aggregators (agents minimum)

Agent : ClaudeResearcher - Primary Name Search```
...
Agent : ClaudeResearcher - Comprehensive Search```
...
Agent : GeminiResearcher - Alternative Spellings```
...
CATEGORY : Social Media Search (agents minimum)

Agent : ClaudeResearcher - LinkedIn/Facebook```
...
Agent : GrokResearcher - Twitter/X Deep Search```
...
Agent : CodexResearcher - Username Enumeration```
...
CATEGORY : Public Records & News (agents minimum)

Agent : ClaudeResearcher - Property/Voter Records```
...
Agent : GeminiResearcher - Court/Business Records```
...
Agent : ClaudeResearcher - News & Mentions```
...
What to Compile from ALL Agent Results:- All addresses found (current and historical)
...
Step : Reverse Lookups on Discovered Info

Description:Use discovered phone/email/username for additional data

Invoke:Read ReverseLookup.md

For Each Phone Number Found:- Run through CallerID, NumLookup
- Cross-reference with people search sites

For Each Email Found:- Run through Holehe (account discovery)
- Check Hunter.io for company email patterns

For Each Username Found:- Run through Sherlock or WhatsMyName
...
Step : Associate Network Mapping

Description:Investigate known relatives and associates for additional leads

Actions:. Search each relative/associate name found in Step . Check their social media for subject mentions/tags
. Look for mutual connections on LinkedIn
. Search for family events (weddings, obituaries) that may mention subject

Associate Search Strategy:- Parents often have more stable addresses
...
Step : Verification & Confidence Assessment

Description:Confirm you've found the correct person

Invoke:Read VerifyIdentity.md

Verification Checklist:- [ ] Age/DOB matches expected range
...
Confidence Scoring:| Score | Criteria |
|-------|----------|
| HIGH | + unique identifiers match from independent sources |
| MEDIUM | identifiers match, timeline consistent |
...
Step : Compile Investigation Report
...
What this workflow produces:- Comprehensive subject profile with all discovered information
- Contact information (address, phone, email) if available
- Social media account URLs
- Confidence assessment
- Investigation report

Deliverable Format:- Markdown report as shown in Step - All sources documented
