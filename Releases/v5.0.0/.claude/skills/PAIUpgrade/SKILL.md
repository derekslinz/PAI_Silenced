---
name: PAIUpgrade
description: "Generate prioritized PAI upgrade recommendations via parallel threads: Thread (prior-work audit — reads current Algorithm, PATTERNS.yaml, hooks, settings, recent ISAs, and KNOWLEDGE to assign Prior Status tags), Thread (user context — TELOS goals, active projects, PAI system state), Thread (source collection — Anthropic releases, YouTube channels, GitHub trending, custom sources), Thread (internal reflections — Algorithm execution Q/Qpatterns). Output format: Discoveries table ranked by interestingness, then tiered Recommendations (CRITICAL/HIGH/MEDIUM/LOW) each with Prior Status (NEW/PARTIAL/DISCUSSED/REJECTED/DONE), then full Technique Details with before/after code. Every recommendation cites file:line evidence from Thread — already-implemented items go to Skipped, never re-surfaced. Workflows: Upgrade, MineReflections, AlgorithmUpgrade, ResearchUpgrade, FindSources, TwitterBookmarks. USE WHEN upgrade, system upgrade, check Anthropic, new Claude features, algorithm upgrade, PAI upgrade, check bookmarks, scan bookmarks, twitter bookmarks, X bookmarks, bookmarks for upgrades, what have I bookmarked, mine reflections."
effort: high
---
...
The skill runs four parallel agent threadsthat converge into personalized recommendations:

- Thread — Prior-Work Audit (Algorithm, PATTERNS.yaml, hooks, skills, recent ISAs, KNOWLEDGE, feedback memory)
- Thread — User Context (TELOS, projects, recent work, PAI state)
- Thread — Source Collection (Anthropic, YouTube, custom sources, GitHub trending)
- Thread — Internal Reflections (algorithm-reflections.jsonl)

Thread is the guard rail against recommending what's already implemented, already researched-and-deferred, or already decided against.Synthesis MUST filter every candidate recommendation through Thread 's state inventory — never emit a recommendation without a Prior Status assignment.
...
Default workflow:If user says "upgrade" or "check for upgrades" without specifics, run the Upgradeworkflow (which includes Thread reflection mining).
...
. Every output item is a TECHNIQUE— a specific pattern, code snippet, configuration, or approach.
. Quote or code-block the actual content— show exactly what was said/written.
. Map to PAI components— every technique connects to a specific file, skill, workflow, or system component.
. Verify Prior State (Thread gate)— Before emitting ANY recommendation, confirm against Thread inventory: is it already in Algorithm / PATTERNS.yaml / hooks / SKILL files / KNOWLEDGE / prior ISAs? Assign a Prior Status emoji and cite evidence. Items that are DONE go to Skipped, not Recommendations.
. Two mandatory description fields, ≤sentences each, concrete and specific:   - What It Is:the technique itself — what it does, how it works, what capability it provides
   - How It Helps PAI:the specific benefit — which component improves, what gap it fills
. Provide implementation— show before/after code or specific steps.
. Skip, don't dilute— if content has no extractable technique, put it in Skipped Content with reason.
...
Skill Files:- `sources.json` — Anthropic sources config (+ sources)
...
| Tool | Purpose |
|------|---------|
| `Tools/Anthropic.ts` | Check + Anthropic sources (blogs, GitHub repos, changelogs, docs) for updates |

Key Principles

. Extract, Don't Summarize— pull specific techniques, never just link to sources.
. Quote the Source— actual code, doc quotes, or transcript excerpts.
. PAI-Contextualized— every technique maps to a specific PAI file, skill, or component.
. Explain "Why You"— "this helps because your [X] currently [Y]".
. TELOS-Connected— reference user's goals and challenges when explaining relevance.
. Skip Boldly— if content has no extractable technique, skip it entirely.
. Implementation-Ready— provide actual code changes, not vague recommendations.
. Claude Code Freshness via claude-code-guide— when discoveries involve Claude Code internals (hooks, settings, slash commands, MCP, agent types, keybindings, Agent SDK, Claude API), spawn `Agent(subagent_type="claude-code-guide")` to verify PAI's current references match the latest API surface.
...
|---------------|----------------|-------------------|
| "Check out R Amjad's video on Claude Code" | Points to content instead of extracting it | "@ :, R Amjad shows this technique: [quote]" |
| "v..has task management improvements" | Vague summary, no technique | "v..adds `addBlockedBy` parameter: [code example]" |
...
| "Top Actions" or flat recommendation list | No priority tiers | Recommendations section with ///tiers, each with PAI Relevance column |
...
| Missing Prior Status column| Bypasses Thread gate | Every recommendation row cites evidence from Thread 's inventory |
...
- MineReflections.md— Thread standalone (deep reflection mining)
...
- Full upgrade check can take -minutes.Use `run_in_background: true` for the outer agent.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"PAIUpgrade","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
