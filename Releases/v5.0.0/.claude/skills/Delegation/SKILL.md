...
These are NOT the same thing:- Custom agents= one-shot parallel workers with unique identities, launched via `Task()`, no shared state
...
- + independent workstreamsexist at Extended+ effort level
- Multiple identical non-serial tasksneed parallel execution
- Specialized expertiseneeded (architecture design, implementation, ISC optimization)
- Large codebase changesspanning + files benefit from parallel workers
- Research + executioncan proceed simultaneously
...
. Custom Agents (via Agents Skill)
...
```
. TeamCreate(team_name="my-project")           Creates team + task list
. TaskCreate(subject="Implement auth module")   Create team tasks
. Task(subagent_type="Engineer", team_name="my-project", name="auth-engineer")  Spawn teammate
. TaskUpdate(taskId="", owner="auth-engineer") Assign task
. SendMessage(type="message", recipient="auth-engineer", content="...")  Coordinate
```

This is a COMPLETELY DIFFERENT system from custom agents:- Custom agents(Agents skill) = fire-and-forget parallel workers, no shared state
- Agent teams(TeamCreate) = persistent coordinated teams with shared task lists, messaging, multi-turn

Team Guidelines:- Use for + independently workable criteria at Extended+
...
Concrete examples:- "Research topics in parallel" → Subagents(independent, no coordination needed)
- "Build a feature spanning API + UI + tests with shared state" → Teams(cross-layer, needs coordination)
- "Run file updates with same pattern" → Subagents(parallel, identical, independent)
- "Debug a complex issue with competing hypotheses" → Teams(need to share findings, adjust approach)

. Parallel Task Dispatch

For N identical operations (e.g., updating files with the same pattern):

. Create N `Task()` calls in a single message (parallel launch)
...
| Standard | -foreground agents max for discrete subtasks |
| Extended | -agents, background agents for research |
| Advanced | -agents, agent teams for + workstreams |
...
```
Task(subagent_type="general-purpose", model="haiku", max_turns=, prompt="...")
```

- Use `model="haiku"` for cost/speed efficiency
- Set `max_turns=` — if it can't finish in turns, it needs full delegation
- Provide all input inline in the prompt (no tool use expected)
- Examples: "Classify this text as X/Y/Z", "Extract the key points from this", "Summarize this in sentences"
...
- Examples: "Research X and produce a report", "Refactor these files", "Debug why test Y fails"
...
Why this matters:Spawning a full agent for a one-shot extraction wastes ~-s of startup overhead and unnecessary context. Lightweight delegation returns in -s. Over an Extended+ Algorithm run with + delegations, this saves minutes. Inspired by RLM's `llm_query()` vs `rlm_query()` two-tier pattern (Zhang/Kraska/Khattab ).

Anti-Patterns (Don't Do These)

- Don't delegate what Grep/Glob/Read can do in <seconds
- Don't spawn agents for single-file changes
- Don't create teams for fewer than independent workstreams
...
- + independent workstreams warrant delegation.For -tasks, direct work is faster than team coordination overhead.
- Agent teams share a task list.Use TaskCreate/TaskUpdate for coordination, not ad-hoc messages.
- Teams overkill for single-file tasks.(Mar reflection: "one agent that can both read code and write JSX is better than three specialists who can't coordinate")
...
User: "launch an agent team to research these topics"
→ Creates team with research agents
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Delegation","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
