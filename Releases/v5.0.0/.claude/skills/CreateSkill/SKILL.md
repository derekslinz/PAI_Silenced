...
Wrong (NEVER use):- Skill dirs: `createskill`, `create-skill`, `CREATE_SKILL` (no underscore + caps for public; no kebab/snake for private)
...
Do not use SKILLCUSTOMIZATIONS to smuggle private content into a public skill.If the skill requiresprivate context to function (real customer name, real API account, real internal infra), it is a private skill — name it `_ALLCAPS` and stop.
...
- Public API endpoints that are conventions, not secrets (e.g., `localhost:/notify`)
...
CRITICAL: Keep folder structure FLAT - maximum levels deep.
The Rule
...
These subdirectories are allowed:- Workflows/- Execution workflows ONLY
...
When to use References/:When SKILL.md exceeds ~lines and has substantial reference content (API signatures, detailed examples, troubleshooting guides). Keep SKILL.md as a routing guide; move encyclopedic content to References/.
...
For skills with SKILL.md > lines:Use dynamic loading to reduce context on skill invocation.
...
SKILL.md= Minimal (-lines) - loads on skill invocation
...
Full Documentation:- Detail : `SkillSearch('skillname detail')` → loads Detail.md
- Detail : `SkillSearch('skillname detail')` → loads Detail.md
```

When To Use

Use dynamic loading for:- SKILL.md > lines
- Multiple documentation sections
- Extensive API reference
- Detailed examples

Don't use for:- Simple skills (< lines)
- Pure utility wrappers (use PAI/TOOLS.md instead)

Benefits

- Token Savings:%+ reduction on skill invocation (when full docs not needed)
...
Before creating any skill, identify which of the types it is (from Anthropic's internal skill taxonomy, Thariq Shihipar, Mar ). The type shapes structure and testing decisions.

| Type | Focus | Key Structure | Example |
|------|-------|---------------|---------|
| . Library/API Reference | Gotchas, edge cases Claude gets wrong | Lightweight, gotchas-heavy, reference snippets | HonoReference, DReference |
| . Product Validation | Test/verify code works | State assertions, browser automation, output recording | Browser |
| . Data Fetching | Connect to data systems | Credential refs, query patterns, dashboard pointers | USMetrics, _METRICS |
| . Business Process | Automate repetitive workflows | Execution logs, consistency tracking | _CLICKUP, _BROADCAST |
| . Code Scaffolding | Generate framework boilerplate | Template files, project-aware scripts | CreateCLI, CreateSkill |
| . Code Quality | Enforce standards, review | Deterministic scripts, hook integration | /simplify, /code-review |
| . CI/CD & Deployment | Deploy with safety patterns | Pre-deploy checks, smoke tests, rollback | (gap — needs Deploy skill) |
| . Operations Runbooks | Map phenomena to diagnostics | Phenomenon → tool → query → report | _HEALTHCHECK |
| . Infrastructure Ops | Maintenance with safety guardrails | Safety gates, audit logging, orphan detection | _PAI, _DOTFILES |

Skill Writing Guidance

When writing or improving skill instructions, follow these principles from Anthropic's skill-creator methodology and Thariq Shihipar's "Lessons from Building Claude Code" (Mar ):
...
- Explain the why, not just the what.Models with good theory of mind + clear reasoning outperform models with rigid constraints. Instead of "ALWAYS use bullets", explain why bullets matter for the audience.
- Keep it lean.The context window is a public good. Remove instructions that don't improve output. If test transcripts show the agent wasting time on unproductive steps, cut them. SKILL.md should be under lines.
...
. Level (YAML frontmatter):Always in system prompt. Triggering info only.
. Level (SKILL.md body):Loaded when skill is invoked. Routing + key guidance.
. Level (Reference files):Root-level `.md` files or `References/` subdirectory loaded on demand.

Tell Claude what files exist; it will read them when appropriate. SKILL.md should be under lines — if over, extract detailed content to reference files.
...
All guidance above derived from Thariq Shihipar's "Lessons from Building Claude Code" (Mar ), Anthropic's official skill guide, and platform documentation.
Examples

Example : Create a new skill from scratch```
...
Example : Fix an existing skill that's not routing properly```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"CreateSkill","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
