...
Step : Read the Authoritative Sources

REQUIRED FIRST:
. Read the skill system documentation: `~/.claude/PAI/DOCUMENTATION/Skills/SkillSystem.md`
. Read a canonical example skill — pick any existing public skill in `~/.claude/skills/` (e.g. `Research/SKILL.md`, `Daemon/SKILL.md`) and study its frontmatter, workflow routing, and examples sections.
...
Step a: Identify Skill Type

Classify the skill using the Anthropic skill types (see Skill Types table in SKILL.md):

| | Type | Key Structural Pattern |
|---|------|----------------------|
| | Library/API Reference | Gotchas-heavy, reference snippets |
| | Product Validation | Browser/tmux, state assertions |
| | Data Fetching | Credentials, query patterns |
| | Business Process | Execution logs, consistency |
| | Code Scaffolding | Templates, project-aware scripts |
| | Code Quality | Deterministic scripts, hook integration |
| | CI/CD & Deployment | Safety gates, rollback, smoke tests |
| | Operations Runbook | Phenomenon → diagnosis → report |
| | Infrastructure Ops | Safety guardrails, audit logging |

The type informs structure decisions — e.g., Type skills are mostly gotchas, Type needs safety gates.

Step b: BPE Check

Before building, apply the bitter lesson test: "Would a smarter model make this skill unnecessary?"
- If the skill provides knowledge Claude can't derive (API quirks, org decisions) → proceed- If the skill provides tools Claude can't replicate (API calls, automation) → proceed- If the skill just orchestrates Claude's reasoning → question whether it's needed
Step : Determine TitleCase Names
...
Wrong naming (NEVER use):- `create-skill`, `create_skill`, `CREATESKILL` → Use `CreateSkill`
- `create.md`, `CREATE.md`, `create-info.md` → Use `Create.md`, `CreateInfo.md`

Step : Create the Skill Directory
...
For large skills (>lines):Consider adding a `References/` subdirectory for detailed API docs, extensive examples, or troubleshooting guides. Keep SKILL.md as a routing guide.

Step b: Public Release Readiness (MANDATORY)
...
. No sensitive content— no API keys, tokens, credentials, private URLs, auth secrets, private data
. No personal references— no author name, no specific project names, no personal domains, no first-person war stories, no user-specific absolute paths like `/Users/<name>/...`
. Generic framing— "someone reports a bug" over "<author-name> reports a bug"; "your web project" over "my UL site"; "a common root cause" over "the Hroot cause"
...
Why this matters:- Tools have rich configuration via flags
...
- [ ] SKILL.md under lines (extract to References/ or root files if over)
...
Step : Suggest Effectiveness Testing

After creating the skill, suggest to the user:

> "The skill structure is ready. Want me to test itto see if it actually improves outcomes? I can run it against real prompts and compare with a no-skill baseline using the TestSkill workflow."
