...
. The skill:Read the target SKILL.md and the specific workflow(s) relevant to the feedback
. Test results(if from TestSkill): Read outputs from `MEMORY/WORK/skill-test-[name]/`
. User feedback:What specific complaints or requests did the user have?
. Transcripts(if available): How did the agent actually use the skill? Where did it waste time or go wrong?

---

Step : Diagnose the Problem
...
Step : Apply the Writing Philosophy
...
Bad:
```
ALWAYS use exactly bullet points. NEVER exceed words per bullet.
...
who need to absorb the main message in under seconds. Keep bullets concise
...
. Edit SKILL.md— Update instructions, description, routing as needed
. Edit workflows— Revise step-by-step instructions
. Add Tools/— If repeated work was identified, create bundled scripts
. Validate structure— Run through the ValidateSkill checklist mentally:
...
Step : Verify and Next Steps

After making changes:

- If coming from TestSkill loop:Return to TestSkill Step to rerun tests with the improved skill. Use a new `iteration-[N+]/` directory.
