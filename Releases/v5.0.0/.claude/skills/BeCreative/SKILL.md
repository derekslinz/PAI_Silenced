...
Enhance AI creativity using deep thinking + Verbalized Sampling. Combines research-backed techniques (Zhang et al., ) for .-.x diversity increase and extended thinking for quality.
...
Running the WorkflowNameworkflow in the BeCreativeskill to ACTION...
```
...
| `Workflows/TechnicalCreativityGemini.md` | "technical creativity", "algorithm", "architecture" | Engineering creativity via Gemini Pro |
...
Core technique:Generate diverse options (p<.each) internally, output single best response.
...
For technical creativity:Use TechnicalCreativityGeminiworkflow.
...
Works well with:- XPost/ LinkedInPost- Generate creative social media content
...
Example : Technical creativity```
User: "deep thinking this architecture problem"
-> Invokes TechnicalCreativityGeminiworkflow
-> Uses Gemini Pro for mathematical/algorithmic creativity
-> Returns novel technical solution
```

---

Research-backed creative enhancement: .-.x diversity, .% quality improvement.
Gotchas
...
- .-.x diversity claims come from specific benchmark conditions.Real-world diversity improvement varies with prompt type.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"BeCreative","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
