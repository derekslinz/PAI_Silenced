...
Extract at different depths depending on need. Default is Fullif no level is specified.

| Level | Sections | Bullets/Section | Closing Sections | When |
|-------|----------|----------------|-----------------|------|
| Instant| | | None | Quick hit. One killer section. |
| Fast| | | None | Skim in seconds. |
| Basic| | | One-Sentence Takeaway only | Solid overview without the deep cuts. |
| Full| -| -| All three | The default. Complete extraction. |
| Comprehensive| -| -| All three + Themes & Connections | Maximum depth. Nothing left behind. |

How to invoke:"extract wisdom (fast)" or "extract wisdom at comprehensive level" or just "extract wisdom" for Full.

Comprehensive extras:- Themes & Connectionsclosing section: identify -throughlines that connect multiple sections. Not summaries — the deeper patterns the speaker may not even realize they're revealing.
- Prioritize breadth. Every significant wisdom domain gets its own section.
- No merging sections to save space. If the content supports sections, use .
...
THREE LEVELS — we're aiming for Level :
Level (BAD — documentation):- The speaker discussed the importance of self-modifying software in the context of agentic AI development
...
Level (BETTER — but still "smart bullet points"):- He built self-modifying software basically by accident — just made the agent aware of its own source code
- Money has diminishing returns. A cheeseburger is a cheeseburger no matter how rich you are.
- "Vibe coding is a slur" — he calls it agentic engineering, and only does vibe coding after am

Level (YES — this is what we want — conversational, the user's voice):- He wasn't trying to build self-modifying software. He just let the agent see its own source code and it started fixing itself.
- Past a certain point, money stops mattering. A cheeseburger is a cheeseburger no matter how rich you are.
- He calls vibe coding a slur. What he does is agentic engineering. The vibe coding only happens after am, and he regrets it in the morning.

The difference between Level and :Level is compressed info with em-dashes. Level is how you'd actually SAY it. Varied sentence lengths. Letting a thought breathe. Not trying to be clever — just being clear and direct and a little bit personal.

Key signals of Level :- Reads naturally when spoken aloud
...
. Write like you'd say it.Read each bullet aloud. If it sounds like a press release or a compressed tweet, rewrite it. If it sounds like you telling a friend what you just watched, you nailed it.
. -words per sentence.This is the target range. Mix short (-) with medium (-) and longer (-). Don't make them all the same length. Exception: verbatim quotes can be any length since they're the speaker's actual words.
. Let ideas breathe.Use periods between thoughts, not em-dashes. Short sentences. Then a slightly longer one to explain. That's the rhythm.
. Include the actual detail.Not "he talked about money" but "a cheeseburger is a cheeseburger no matter how rich you are."
. Use the speaker's words when they're good.If they said something perfectly, use it.
. No hedging language.Not "it was suggested that" or "the speaker noted." Just say the thing.
. Capture what made you stop.Every bullet should be something worth telling someone about.
. Vary your openers.Don't start three bullets the same way. And don't front-load with "He" — if more than bullets in a section start with the speaker's name, you're writing a biography.
. Capture the human moments.Burnout stories, moments of doubt, something that moved them. That's wisdom too. Don't skip it because it's not "technical."
. Insight over inventory."He uses Go for CLIs" is inventory. "He picked a language he doesn't even like because the ecosystem fits agents perfectly. That's the new normal." is insight. Go deeper.
. Specificity is everything."He was impressed by the agent" = bad. "The agent found ffmpeg, curled the Whisper API, and transcribed a voice message nobody taught it to handle" = good.
. Tension and surprise.The best bullets have a contradiction or reversal. "Every VC is offering hundreds of millions. He genuinely doesn't care." The gap between the offer and the indifference IS the wisdom.
. Understated, not clever.Let the content carry the weight. You don't need to manufacture drama or craft the perfect one-liner. Just state what's interesting plainly and move on.
...
Phase : Section Selection

Pick sections based on depth level (default Full = -). Requirements:
- Section count follows depth level table. Full = -, Comprehensive = -, Basic/Fast = , Instant = .
- Each section must have at least STRONG bullets to justify existing (except Fast, where tight bullets IS the section). If you can only scrape together weak ones, merge into a related section.
...
- Name sections like a magazine editor."The Death of % of Apps" is great. "Technology Predictions" is not. The section name itself should make you curious. It's a headline, not a category.
- Surprise density per section.If a section has + bullets but only are genuinely surprising, kill the padding and keep the winners. Quality > quantity per section.
...
For each section, extract -bullets depending on density. Apply all tone rules. Every bullet earns its place.
...
Phase : Closing Sections (Depth-Level Dependent)
...
| Full| One-Sentence Takeaway + If You Only Have Minutes + References & Rabbit Holes |
| Comprehensive| All three above + Themes & Connections |

One-Sentence TakeawayThe single most important thing from the entire piece in -words.

If You Only Have MinutesThe -absolute must-know points. The cream of the cream.

References & Rabbit HolesPeople, projects, books, tools, and ideas mentioned that are worth following up on. Brief context for each.

Themes & Connections(Comprehensive only)
-throughlines that connect multiple sections. The deeper patterns the speaker may not realize they're revealing. Not summaries. Synthesis.
...
- [ ] Section count matches depth level (Instant=, Fast/Basic=, Full=-, Comprehensive=-)
- [ ] Closing sections match depth level (see Phase table)
- [ ] No bullet starts with "The speaker" or "It was noted that"
- [ ] No more than bullets per section start with "He" or the speaker's name
- [ ] No bullet exceeds words
- [ ] No inventory sections (just listing facts without insight)
- [ ] "If You Only Have Minutes" bullets are each under words
...
- YouTube extraction should use `fabric -y URL` firstto get the transcript before extracting wisdom.
- Long content may need chunking.Don't try to extract wisdom from a -hour podcast transcript in one pass.

Examples

Example : YouTube interview extraction```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"ExtractWisdom","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
