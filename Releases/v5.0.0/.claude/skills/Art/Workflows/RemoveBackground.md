...
bun ~/.claude/PAI/TOOLS/RemoveBg.ts img.png img.png img.png
```
...
Step : Replace or Copy to Destination
...
Example : Remove background from a diagram

```bash
bun ~/.claude/PAI/TOOLS/RemoveBg.ts ~/Downloads/TheAlgorithm.png
```

Example : Remove background and save with new name
...
Solution:Expected. The default `unet` model (~MB) is fetched once into `~/.unet/`, then cached forever. Subsequent runs are fast.
...
```bash
~/.local/bin/rembg i -m unetp input.png output.png       smaller/faster
...
Solution:Use `birefnet-general` (or `birefnet-portrait` for people) — both produce noticeably better edges than the default `unet`.
...
Last Updated:--— switched from poof.bg to local rembg
