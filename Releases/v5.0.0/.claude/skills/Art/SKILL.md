---
name: Art
description: "Generates static visual content across + formats via Flux, Nano Banana Pro (Gemini Pro), and GPT-Image-. Covers blog header illustrations, editorial art, Mermaid flowcharts, technical architecture diagrams, D.js dashboards, taxonomies, timelines, xframework matrices, comparisons, annotated screenshots, recipe cards, aphorism/quote cards, conceptual maps, stat cards, comic panels, YouTube thumbnails, PAI pack icons, and brand-logo wallpapers. Named workflows: Essay, DDashboards, Visualize, Mermaid, TechnicalDiagrams, Taxonomies, Timelines, Frameworks, Comparisons, AnnotatedScreenshots, RecipeCards, Aphorisms, Maps, Stats, Comics, YouTubeThumbnailChecklist, AdHocYouTubeThumbnail, CreatePAIPackIcon, LogoWallpaper, RemoveBackground. SKILLCUSTOMIZATIONS loads PREFERENCES.md, CharacterSpecs.md, and SceneConstruction.md. --remove-bg flag produces transparent-background PNG (can produce black backgrounds — verify visually). Up to reference images per request (human, object Gemini API limit). Output staged to ~/Downloads/ for preview before any project directory copy. Nano Banana Pro uses --size for resolution tier K/K/K and separate --aspect-ratio. USE WHEN: art, illustration, diagram, flowchart, infographic, header image, thumbnail, visualize, generate image, mermaid, architecture diagram, comic, icon, blog art, framework diagram, Dchart, remove background, wallpaper. NOT FOR video or animation (use Remotion). NOT FOR the user's personal portrait/headshot (use a dedicated headshot skill)."
effort: medium
---
...
|---------------|-------------------|
| Blog header / editorial essay illustration | `Workflows/Essay.md`— Steps –in order, no skipping |
| Mermaid diagram | `Workflows/Mermaid.md` |
| Technical / architecture diagram | `Workflows/TechnicalDiagrams.md` |
| Framework / x/ matrix | `Workflows/Frameworks.md` |
| Ddashboard / chart | `Workflows/DDashboards.md` |
...
If no workflow matches the request, stop and surface to the userbefore generating — propose either (a) the closest existing workflow, (b) using `Visualize.md` as the generic catch-all, or (c) creating a new workflow first via the `CreateSkill` skill. Do not improvise.
...
Enforcement when calling `Generate.ts`:- `--thumbnail` is the ONLY correct flag for blog headers — it implicitly enables `--remove-bg` and produces BOTH `output.png` (transparent) AND `output-thumb.png` (EAEDF background).
...
Verification step before declaring an image done (REQUIRED):. `file ~/Downloads/[name].png` → must report `PNG image data, ... RGBA` (-bit/color RGBA). If it says `JPEG` or `-bit colormap` without alpha, transparency failed.
. `file ~/Downloads/[name]-thumb.png` → must report `PNG image data`. The thumb is intentionally opaque with sepia background.
. Only after both pass: copy to the project directory and wire into the post.

Wiring into the blog post:- Body inline: `[![Alt](/images/blog/[slug]/header.webp)](/images/blog/[slug]/header.webp)` — use the transparent WebP converted from the `.png`.
...
Route to the appropriate workflow based on the request.
  ...
  - D.js interactive chart or dashboard → `Workflows/DDashboards.md`
  ...
  - Framework or xmatrix → `Workflows/Frameworks.md`
...
User customizationdefines specific aesthetic preferences including:
...
User customizationmay include reference images for consistent style.
...
Fallback:nano-banana-pro (Gemini Pro)
...
| `nano-banana-pro` | Resolution tier | `K`, `K`, `K` (also accepts `--aspect-ratio` separately) | `K` |
| `gpt-image-` (current) / `gpt-image-.` (fallback) | Pixel dimensions | `x`, `x`, `x`, `x`, `auto` | NOTE: `gpt-image-` is deprecatedper OpenAI docs — do not use. | `x` |

Note:`nano-banana-pro` uses `--size` for resolution quality and a separate `--aspect-ratio` flag for aspect ratio (defaults to `:`).
...
Workflow:. Generate to `~/Downloads/[descriptive-name].png`
...
API Limits (Gemini):- Up to human reference images
...
→ Generates K image with --remove-bg for transparency
→ Resizes to xRGBA PNG
...
- nano-banana-pro uses `--size` for resolution (K/K/K) and SEPARATE `--aspect-ratio` flag.Don't pass aspect ratio values to `--size`.
- Reference images: max human, object, total per request(Gemini API limit).
...
- When asked to use a specific image URL or file, use EXACTLY that asset.Don't substitute similar images. Past rating-failures from using wrong image assets.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Art","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
