...
TMP=$(mktemp -d)
cp -r "$REPO/tailwind.config." "$TMP/" >/dev/null
cp -r "$REPO/src/styles" "$TMP/" >/dev/null
cp -r "$REPO/src/components/ui" "$TMP/" >/dev/null
cp "$REPO/package.json" "$TMP/" >/dev/null
```
...
. Wait for System Extraction

Claude Design takes s-min depending on codebase size. Output appears in the design system panel with:
...
- Feeding a full repo— Claude Design gets overwhelmed by K+ files. Curate a focused bundle (~-files max).
...
- One-shot expecting perfection— extraction is iterative. Plan for -refinement rounds before the system is solid.

Time Estimate

-minutes depending on codebase complexity and conflict count.
