...
- Reference images— -local image paths for visual inspiration
...
Wait for Claude Design to produce the first version (typically -seconds on Opus .).
...
mkdir -p "$OUT"
bun ~/.claude/skills/Webdesign/Tools/DriveClaudeDesign.ts screenshot "$OUT/v.png"
```

Review the screenshot. If it matches the brief, proceed to step . If not, hand to `RefinePrototype.md`.
...
-minutes for a single-iteration prototype. Add -minutes per refinement round.
