...
Use when:you have a pool of candidate ideas and want them scored on the standard dimensions (Feasibility, Novelty, Impact, Elegance). No breeding, no selection, no iteration — just scoring.
...
- Judge count(optional, default ): number of judge agents per candidate
...
. Optional adversarial pass:if `redteam_pass` is true, invoke `Skill("RedTeam")` to attack each candidate. Surfaced fatal flaws are appended to the candidate metadata before scoring (judges see them).

. Spawn `judge_count` Judge agents in parallelvia Task tool. Each judge independently scores ALL candidates. Trait composition: `critical + analytical + skeptical`.

. Each judge scores each candidate on dimensions (-each):
   ...
   - Score (-)
   - -sentence supporting argument
   - -sentence counterargument

. Aggregate across judges:   - Final score per dimension = average across judges
   - Composite score = average of dimension scores
   - Confidence = inverse of judge variance (high variance = low confidence)

. External validation (optional):if any hooks are enabled, run them on each candidate. Hook returns `{ modifier: -..+, evidence: string }`. Adjusted composite = base composite + sum of modifiers (capped to ±).
...
[
  {
    ...
    "external_validation": {
      "market_search": { "modifier": -, "evidence": "Partial prior art in gossip protocols" }
    },
    "adjusted_composite": .  }
]
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Ideate","workflow":"Test","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
