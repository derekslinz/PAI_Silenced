...
Human creativity reduces to irreducible functions:
...
The workflow phases expand these into a richer human-legible system. DREAM, DAYDREAM, and CONTEMPLATE are PERTURB at different noise levels. MATE is PERTURB on existing ideas. META-LEARN adds the Lamarckian advantage — analyzing WHY ideas worked and steering future generation.

The Phases (Summary)

| | Phase | Noise | What it does | Agent |
|---|-------|-------|--------------|-------|
| | CONSUME| — | Multi-domain research, atomic idea extraction | The Glutton |
| | DREAM| .| Free-association on random input subsets, no problem awareness | The Dreamer |
| | DAYDREAM| .| Tangential wandering with the problem held loosely | The Wanderer |
| | CONTEMPLATE| .| Structured analysis via lenses (mandatory; checkpoint A gates) | The Sage |
| | STEAL| — | Cross-domain pattern borrowing via weighted random domain lottery | The Thief |
| | MATE| — | Genetic recombination via Fisher-Yates shuffle + mutation operations | The Matchmaker |
| | TEST| — | Multi-judge scoring on Feasibility/Novelty/Impact/Elegance (checkpoint B gates) | The Judge |
| | EVOLVE| — | Selection: kill bottom %, elite top %, mutate the rest, immigrant injection | The Curator |
| | META-LEARN| — | Lamarckian strategy adjustment + next-cycle question generation | The Scientist |

Post-loop: The Historianruns the Insight Extractor for cross-cycle pattern analysis.
...
| User says... | Workflow |
|--------------|----------|
| "ideate", "id", "novel ideas for X", "evolve ideas for X", default | `Workflows/FullCycle.md` |
...
```json
{
  ...
  "fitness_history": [{"cycle": , "avg_score": ., "top_score": ., "diversity_index": .}],
  "stagnation_counter": ,
  "strategy_version": ,
...
Loop Gate logic:```
IF budget_seconds_remaining <= :        STOP (budget exhausted)
ELIF stagnation_counter >= :
    IF strategy_pivots_remaining > :    PIVOT (shift domains/noise/agents)
    ELSE:                                STOP (exhausted strategies)
ELIF diversity_index < .:              PIVOT (collapse — inject immigrants)
...
LLM "temperature" is soft probability redistribution biased toward the training distribution. Ideate uses structural randomnessat the data level instead:

- Input subsetting(DREAM): Fisher-Yates shuffle picks each agent's input subset
- Domain lottery(STEAL): weighted random sampling from the + candidate domain pool
- Pairing shuffle(MATE): Fisher-Yates pairs adjacent items; % slots forced cross-phase
- Mutation dice(EVOLVE): roll an -sided die, apply that mutation operation:
  . Flip one assumption
...
```json
{
  ...
  "scores": {
    "feasibility": , "novelty": , "impact": , "elegance": ,
    "composite": ., "confidence": ., "judge_variance": .,
    "external_validation": {"market_search": {"modifier": -, "evidence": "..."}},
    "adjusted_composite": .  },
...
The idea:[-sentences]
Scores:Feasibility: | Novelty: | Impact: | Elegance: External validation:[hook results]
...
```json
{
  "problem": "...",
  "time_scale": "weeks",
  "domains": ["primary", "adjacent-", "adjacent-"],
  "scoring_weights": {"feasibility": ., "novelty": ., "impact": ., "elegance": .},
  "convergence_prevention": {
    "cross_phase_breeding_min": .,
    "immigrant_ideas_per_cycle": ,
    "kill_threshold": .,
  ...
  "randomness": {"seed": null, "subset_ratio": ., "mutation_operations": }
}
```
...
| IterativeDepth | CONTEMPLATE | -lens analysis (Literal, Failure, Analogical, Constraint Inversion) |
...
- The -phase decomposition and the path-to-ASI mapping derive from a publicly published essay on cognitive progress and a possible path to ASI by D. {{PRINCIPAL_SURNAME}} (). The framework name Cognitive Progress Workflowrefers to that essay.
- The Lamarckian advantage framing (Phase META-LEARN) borrows from research on auto-research loops and meta-learning in agent systems (cf. Karpathy auto-research pattern).
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Ideate","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
