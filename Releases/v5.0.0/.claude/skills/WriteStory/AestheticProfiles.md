...
. The writer selects a base profile during Interview or BuildBible
. Profiles can be blended (e.g., "% Adams, % Tolkien")
. Profiles affect the Prose layer but NOT the other six layers
. Custom profiles can be defined in WriteStory SKILLCUSTOMIZATIONS
...
Characteristics:- Sentences alternate between conversational and grandly philosophical
...
Sentence length:Highly variable (-word punches alternating with -word digressions)
...
Sample register:> The ships hung in the sky in much the same way that bricks don't.
...
Characteristics:- Longer, flowing sentences with subordinate clauses
...
Sentence length:Generally long (-words), with occasional short declarative sentences for impact
...
Sample register:> The world is indeed full of peril, and in it there are many dark places; but still there is much that is fair, and though in all lands love is now mingled with grief, it grows perhaps the greater.
...
Characteristics:- Every word earns its place — nothing wasted
...
Sentence length:Short to medium (-words), rarely ornate
...
Sample register:> The only thing that makes life possible is permanent, intolerable uncertainty; not knowing what comes next.
...
Characteristics:- No quotation marks for dialogue
...
Sample register:> He walked out in the gray light and stood and he saw for a brief moment the absolute truth of the world.
...
Characteristics:- Observation comedy applied to fantasy tropes
...
Sentence length:Medium (-words), conversational rhythm
...
Sample register:> The truth may be out there, but the lies are inside your head.
...
Characteristics:- Each chapter filtered through a specific character's perception
...
Characteristics:- Willingness to break narrative conventions (second person, present tense)
...
Characteristics:- [Sentence structure preference]
- [Vocabulary range and register]
- [Key techniques or conventions]
- [Unique structural choices]
- [Thematic emphasis]

Rhetorical figure emphasis:[-primary figures from RhetoricalFigures.md]
...
Sample register:> [One representative sentence that captures the voice]
```
