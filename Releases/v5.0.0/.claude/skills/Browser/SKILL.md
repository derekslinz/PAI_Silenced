...
. WORK
agent-browser snapshot                    ay tree with @eN refs (for AI)
agent-browser click @e                 click by ref
agent-browser fill @e"hello"           fill input by ref
...
First-time setup (headed, one-time):```bash
...
Subsequent runs (headless, automatic):```bash
...
echo '[["open","https://example.com"],["snapshot"],["click","@e"]]' | agent-browser batch
```
...
Device emulation
agent-browser --device "iPhone " open https://example.com
...
When you need parallel or background browser work (scraping multiple pages, monitoring), spawn general-purpose agentswith browser instructions. No dedicated browser agent type needed — this skill IS the expertise.
...
```
Agent : agent-browser --session scrapeopen https://site-a.com
Agent : agent-browser --session scrapeopen https://site-b.com
```

Fallback:If agent-browser fails or the site has bot detection, use the Interceptorskill instead.

Legacy built-in agents — DEPRECATED, do not invoke.BrowserAgent and UIReviewer are Claude Code built-ins whose internals cannot be modified; they run browser automation that PAI no longer uses. Route all browser work through the Interceptorskill (verification, authenticated flows) or agent-browser(headless scraping).
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Browser","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
