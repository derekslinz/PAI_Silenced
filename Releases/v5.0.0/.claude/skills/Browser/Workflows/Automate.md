...
. User overrides— explicit key=value pairs from the user's input
. Recipe defaults— from frontmatter `defaults` section
. `{PROMPT}`— always replaced with the user's natural language input
. `{URL}`— from user input (commonly the first URL mentioned)
