...
Polarity test for arrows:- Change A. Does B change in the same direction? → (+)- Change A. Does B change in the opposite direction? → (−)
Loop-type test (count the minus signs):- Even number (including ) of (−) arrows in the loop → Reinforcing (R)- Odd number of (−) arrows in the loop → Balancing (B)
Execution

Step : State the Question
...
List -variables — quantities that can increase or decrease over time.

Variables must be:- Nouns or noun phrases, not verbs ("Team size" not "Hiring")
...
Fewer, high-quality variables beat exhaustive lists. Start with the -that matter most; add as needed.

Step : Draw the Arrows
...
. Count the (−) arrows
. Label Rif even, Bif odd
. Give it a short, descriptive name ("Success-to-success," "Coordination tax," "Capacity drift")

Every loop has a name.Un-named loops are untracked. The name captures the dynamic, not the variables.

Step : Interpret the Dynamics
...
Step : Stress-Test Interventions
...
LOOPS:
- R"Success-to-success": A → B → C → A (reinforcing)
  Dynamic: amplifies early wins, accelerates until limit
- B"Coordination tax": A → D → E → A (balancing)
...
```
QUESTION: Why does engineering velocity plateau past team size -?
...
LOOPS:
- R"Success → hiring → output": Team size → Total output → Revenue → Hiring budget → Team size (reinforcing; drives growth)
- B"Coordination tax": Team size → Coordination cost → Per-engineer output → Total output → (pulls back against Rvia less hiring demand pressure)
- B"Onboarding drag": Team size → Onboarding load → Per-engineer output → Total output (balancing, delayed)

DOMINANT LOOP: Rdominant early; Band Bdominant once team > .
...
- Recommended: Don't attack R(slowing hiring just slows the dynamic). Attack Bdirectly: invest in coordination mechanisms (async docs, modular architecture, team topology) that break the "team size → coordination cost" arrow.
```
...
Skill("Art", "Mermaid flowchart showing causal loop diagram with Rand Bloops, variables: [list], arrows with polarity: [list]")
```

Common Mistakes

- Too many variables.A -variable CLD is illegible. Start with -; add only if the loops don't reproduce observed behavior.
...
- Confusing event causation with structural causation."The deploy caused the outage" is event-level; the CLD should show why the deploy processcauses outages repeatedly.
...
- Called from Icebergwhen Layer structure is a feedback loop.
- Renderable via Art— Mermaid / diagram output.

Attribution

Causal Loop Diagrams: Jay Forrester (Industrial Dynamics, ), Dennis Meadows et al. (Limits to Growth, ), Peter Senge (The Fifth Discipline, ). Modern reference: John Sterman, Business Dynamics().
