...
Structured analysis of complex systems — the tools that reveal why the same problem keeps coming backand where a small change produces a large result. Grounded in Donella Meadows, Peter Senge, Jay Forrester, Russell Ackoff, and the Santa Fe Institute tradition.

Systems thinking is the difference between treating symptoms (patch the bug) and fixing structure (change the feedback loop that keeps producing the bug). Most "debug it harder" attempts fail because they operate at the event layer; the real cause lives -layers below, in the structure that generates events.

Core Concept

A systemis a set of elements interconnected in a way that produces a characteristic behavior over time. Change the elements, often nothing happens. Change the interconnections or the purpose, and behavior shifts dramatically.
...
What you win:
- Structural causesinstead of blame-the-nearest-event. The real lever is almost never where the symptom appeared.
- Archetype recognition— most organizational and technical pathologies match one of ~patterns. Naming the pattern unlocks the canonical intervention.
- Leverage-point identification— Meadows' leverage points, ordered. Parameters are low leverage; paradigms are highest. Knowing where to push is the whole game.
...
| FindLeverage| "leverage point", "where to intervene", "highest-leverage change", Meadows' | `Workflows/FindLeverage.md` |
| ConceptMap| "concept map", "map the entities", "relationship map", Novak-style mapping | `Workflows/ConceptMap.md` |

Quick Reference

- workflows— Iceberg, CausalLoop, FindArchetype, FindLeverage, ConceptMap
...
- Archetype count:~canonical patterns (Senge, Braun)
- Leverage points:levels, from parameters (weakest) to paradigm transcendence (strongest) — Meadows

Context files (loaded on demand):- `Foundation.md` — Meadows, Senge, Forrester, Ackoff, Capra; canonical definitions
- `Archetypes.md` — the systems archetypes with structure, recognition signs, canonical intervention
- `LeveragePoints.md` — Meadows' leverage points with worked examples

Integration

Depends on:nothing — standalone analytical skill.

Works well with:- RootCauseAnalysis— RCA is event-layer and pattern-layer; SystemsThinking continues down to structure and mental models. Pair them for deep incident analysis.
...
Example : Strategy```
User: "why does adding engineers slow us down past team size ?"
...
Example : Unintended consequences preview```
...
Attribution:Frameworks drawn from Donella Meadows (Thinking in Systems, ; "Places to Intervene in a System," ), Peter Senge (The Fifth Discipline, ), Jay Forrester (Industrial Dynamics, ), Russell Ackoff (Systems Thinking for Curious Managers), Fritjof Capra (The Web of Life), and the System Dynamics Society tradition.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"SystemsThinking","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
