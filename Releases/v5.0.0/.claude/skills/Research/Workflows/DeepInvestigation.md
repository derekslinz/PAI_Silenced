...
```
Iteration : Broad landscape → discover entities → score them → deep-dive the top one
Iteration : Read previous artifacts → pick next highest-value entity → deep-dive
Iteration +: Continue until coverage gates pass
```
...
Step : Detect Iteration State

```
READ vault directory for existing artifacts:
  - LANDSCAPE.md exists? → This is a CONTINUATION (skip to Step or )
  - ENTITIES.md exists? → This is a CONTINUATION (skip to Step or )
  - Neither exists? → This is FIRST ITERATION (start at Step )

IF continuation:
  READ ENTITIES.md → check for PENDING entities with CRITICAL/HIGH value
  IF PENDING CRITICAL/HIGH exist → skip to Step (Investigate)
  IF all CRITICAL/HIGH done but categories incomplete → skip to Step (Discover)
...
Step : Landscape (Broad — First Iteration Only)
...
Launch Extensive Research (explorers + verifiers = agents):
```
Use the Extensive Research pattern (explorers + verifiers):
...
Overview
[-paragraph synthesis of the domain]
...
- Category : [description, estimated entity count]
- Category : [description, estimated entity count]
- ...
...
Coverage
- Categories: /{N} complete
- Entities: RESEARCHED / {N} total
- CRITICAL/HIGH: RESEARCHED / {N} pending
```

---

Step : Evaluate (Score Entities)

For each PENDING entity without a VALUE score:
Assess on two dimensions:

VALUE (Market/Domain Impact):- CRITICAL— Market leaders, category definers, essential to understanding the domain
...
EFFORT (Information Accessibility):- EASY— Public companies, abundant documentation, press coverage
- MODERATE— Good web presence, some proprietary info
- HARD— Limited public info, stealth-mode, minimal coverage

Priority Order for Investigation:. CRITICAL + EASY (highest ROI)
. CRITICAL + HARD (must-have despite difficulty)
. HIGH + EASY (good ROI)
. HIGH + HARD (worthwhile if time allows)
. MEDIUM+ only after all CRITICAL/HIGH done

Update ENTITIES.mdwith VALUE and EFFORT scores.

---

Step : Discover (Expand Coverage)

Goal:Find entities in undercovered categories.

Check ENTITIES.md for categories with fewer than entities. For each thin category:

Launch -targeted researcher agents:```
Task({
  subagent_type: "PerplexityResearcher",
  prompt: "Find -notable {entity_category} in the {domain} space.
...
Add discoveries to ENTITIES.mdwith status PENDING, then run Step (Evaluate) on them.

---

Step : Investigate (Deep Dive — One Entity)

Goal:Create a comprehensive profile of ONE entity. Quality over quantity.

Select the highest-priority PENDING entity:. Sort by VALUE (CRITICAL first), then EFFORT (EASY first)
. Pick the top one

Load the profile templatefrom the domain template pack for this entity's category.

Launch focused research (agents, entity-specific):
```
Task({
  ...
  prompt: "Deep research on {entity_name} in the context of {domain}.
           ...
           Context: {-paragraph from LANDSCAPE.md about this entity's category}
...
Task({
  ...
  prompt: "Find recent information about {entity_name}:
           ...
           Focus on developments in the last months.
           Tag each finding with confidence: [HIGH], [MED], or [LOW]."
})
...
Task({
  ...
  prompt: "Research {entity_name}: competitive position, strengths, weaknesses,
           how they compare to {list -related entities from ENTITIES.md}.
           What makes them distinctive in the {domain} landscape?
           Tag each finding with confidence: [HIGH], [MED], or [LOW]."
})
```

Produce entity profileusing the domain template:
...
Step .: Verify Entity Profile (Loop Mode Only)

In loop mode, before the Progress Check, spot-check the entity profile for quantitative claims:

. Scan the profile for numbers, dates, and percentages
. Pick the -most important quantitative claims
. Verify each via a quick WebSearch or WebFetch
. Tag verified claims `[HIGH]`, unverifiable claims `[LOW]`
. Note any `[CONFLICT]` items where agents disagreed

In single-run mode, skip this step — verification happens at the end when reviewing the full vault.

This adds ~-s per iteration but catches errors before they propagate into cross-linked profiles.
---

Step : Progress Check (Loop Gate)

Two gates must BOTH pass to exit:
Breadth Gate:```
For each entity category defined in Step :
  Count entities with status != SKIP
  PASS if count >= for ALL categories
  FAIL if any category has < entities
```
...
IF either gate FAILS:
  ...
  → Next iteration re-enters this workflow at Step (which detects continuation)
```
...
|--------|-----------|-----------|
| Iterations | | Algorithm-controlled (N turns) |
| Coverage | Landscape + first deep dive | Full breadth + depth gates |
| Exit | After Step completes | After Step gates pass |
| Best for | Quick overview + top entity | Comprehensive investigation |
| Time | -minutes | -minutes (varies by domain) |

In single-run mode:Complete Steps -(landscape through one deep dive), then report what was accomplished and what remains PENDING for a future loop-mode run.

In loop mode:The Algorithm iterates. Each iteration enters at Step , detects state, and does the next unit of work. Typical iteration pattern:
- Iteration : Steps -(landscape, discover, evaluate, first deep dive)
- Iteration -N: Steps →(detect state, maybe discover more, evaluate, deep dive next)
- Final iteration: Step →(detect state, gates pass, produce summary)
...
. Entity categoriesfor this domain (what types of things to discover)
. Profile templatesper category (what fields to research for each type)
. Evaluation criteria(what makes something CRITICAL vs LOW in this domain)
. Search strategies(domain-specific search tips for researchers)

Available packs:- `MarketResearch.md` — Companies, Products, People, Technologies, Trends
- `ThreatLandscape.md` — Threat Actors, Campaigns, TTPs, Vulnerabilities

No template match?The workflow dynamically creates entity categories based on the landscape research in Step . Templates improve quality but aren't required.
