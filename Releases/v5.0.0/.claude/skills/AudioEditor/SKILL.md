...
[Edit] ffmpeg executes cuts:
    |   - ms qsin crossfades at every edit point
    |   - Room tone extraction and gap filling
    |   - Breath attenuation (% volume, not removal)
    |
...
Example : Clean a podcast recording```
User: "clean up the audio on this podcast file"
-> Invokes Clean workflow
-> Runs full pipeline: transcribe -> analyze -> edit
-> Outputs cleaned MPwith filler words, stutters, and dead air removed
```

Example : Preview edits before applying```
...
Example : Aggressive clean with cloud polish```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"AudioEditor","workflow":"WORKFLOW_USED","input":"_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
