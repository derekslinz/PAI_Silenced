 ...
 * working with {{PRINCIPAL_NAME}}. Opinions evolve based on evidence from sessions.
 *
 * USAGE:
 *   bun OpinionTracker.ts add "{{PRINCIPAL_NAME}} prefers concise responses" --category communication
 *   bun OpinionTracker.ts evidence "{{PRINCIPAL_NAME}} prefers concise responses" --supporting "Got positive reaction to brief answer"
 *   bun OpinionTracker.ts evidence "{{PRINCIPAL_NAME}} prefers concise responses" --counter "Long explanation was appreciated"
 *   bun OpinionTracker.ts list
 *   bun OpinionTracker.ts show "{{PRINCIPAL_NAME}} prefers concise responses"
 *
 * CONFIDENCE UPDATE RULES:
 * - Each supporting instance: +0.02 (capped at 0.99)
 * - Each counter instance: -0.05
 * - Explicit confirmation from {{PRINCIPAL_NAME}}: +0.10
 * - Explicit contradiction from {{PRINCIPAL_NAME}}: -0.20
...
const CONFIDENCE_ADJUSTMENTS = {
  supporting: 0.02,
  counter: -0.05,
  confirmation: 0.10,    // Explicit "yes that's right" from {{PRINCIPAL_NAME}}
  contradiction: -0.20,  // Explicit "no that's wrong" from {{PRINCIPAL_NAME}}
};
