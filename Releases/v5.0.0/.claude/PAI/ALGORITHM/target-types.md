...
What to read:
. `SKILL.md` — purpose, routing description, constraints
. `Workflows/` directory — all workflow files
. `Context/` directory — domain knowledge files
. Any `.md` in the skill root — additional configuration
...
- Generate - diverse inputs spanning the skill's stated capabilities
...
. Copy all skill files to `sandbox/` directory
. For each test input, invoke using `Skill()` tool call with the sandbox copy
. Capture full output text for judging
...
What to read:
. The prompt file itself — full content
. Any sibling files referenced by the prompt (imports, includes)
. File name and directory context for purpose inference
...
- Generate - representative inputs that exercise different aspects
- Include one "stress test" input (unusual format, edge case topic)

How to run in sandbox:
. Copy prompt file to `sandbox/`
. For each test input, send `prompt_content + test_input` to LLM via PAI Inference Tool
. Capture output text for judging
...
What to read:
. The agent definition file — frontmatter + instructions
. Any referenced tool configurations
. System prompt sections if present
...
- Generate - tasks of varying complexity within the agent's stated scope
- Include one task near the agent's capability boundary

How to run in sandbox:
. Copy agent definition to `sandbox/`
. For each test input, spawn agent with the sandbox definition and test task
. Capture agent's final output for judging (not intermediate tool calls)
...
What to read:
. All files listed in `mutable_files`
. Related test files if they exist
. Build/run configuration files
...
How to run:
. Use existing git branch approach: `optimize/{metric_name}`
. Run `metric_command`, extract numeric value
. Compare to baseline
...
. The function signature, arguments, return type
. Calling code — how the function is used
...
When the optimizer enters Phase  (TARGET ANALYSIS), it generates eval criteria automatically:

. Read target comprehensively — load all files per the "What to read" section above
. Extract purpose — what is this target supposed to do?
. Extract constraints — what rules or format requirements exist?
. Extract edge cases — what unusual inputs or states should it handle?
. Draft - binary criteria using the per-type ISC generation template as a starting point, customized to the specific target
. Draft - anti-criteria based on common failure modes for this target type
. Apply the -question test (from eval-guide.md) to each criterion — drop any that fail
. Present to user for approval — show generated criteria, allow modification before loop starts

 Test Input Generation Process

. Read target to understand expected input format and domain
. Generate - diverse inputs spanning the target's capabilities
. Ensure variety:
...
. Present to user alongside eval criteria for approval

 Cross-Type Concerns

All target types share:
- The same -step loop structure (from optimize-loop.md)
