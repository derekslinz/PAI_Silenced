...
| Eval criteria | N/A — metric_command is the signal | - binary yes/no questions judged by LLM |
...
|-----------|-------|---------|--------|
| `stepSize` | .–. | . | How large a change per experiment. Low = tiny tweaks, high = structural changes. |
| `regressionTolerance` | .–. | . | Willingness to accept temporary score regression.  = never,  = freely explore. |
| `earlyStopPatience` | – |  | Consecutive no-improvement experiments before terminating. |
| `maxIterations` | – |  | Maximum total experiments. |

Named presets:

| Preset | stepSize | regressionTolerance | earlyStopPatience | maxIterations | Use For |
|--------|----------|--------------------|--------------------|---------------|---------|
| `cautious` | . | . |  |  | Production systems, stability critical |
| `standard-optimize` | . | . |  |  | Default, moderate approach |
| `aggressive` | . | . |  |  | Prototypes, experiments, local optima escape |
...
- `regressionTolerance` → DECIDE: At ., any regression triggers revert. At .-., minor regressions (<%) accepted with structural rationale. At .+, simulated annealing behavior — accepts regression to escape local optima.
- `earlyStopPatience` → Termination + Plateau Protocol: Adjusts the stagnation thresholds. Plateau Level  at `earlyStopPatience` experiments, Level  at `×earlyStopPatience`, Level  at `×earlyStopPatience`.
...
```
TARGET ANALYSIS:
  . Read target path(s) — single file or directory
  . Detect target type using rules from target-types.md:
     - skill: has SKILL.md
  ...
  . Read target comprehensively per target-types.md "What to read" section
  . If eval mode:
     a. Extract purpose, inputs, outputs, constraints, edge cases
     b. Auto-generate eval criteria (see below)
     c. Auto-generate diverse test inputs (- representative inputs)
  ...
  . Create results.tsv with header and baseline row
  . Write baseline to ISA frontmatter
```

Auto-ISC Generation (eval mode only):

. Read target comprehensively per target-types.md
. Draft - binary eval criteria using the per-type ISC generation template from target-types.md as a starting point, customized to the specific target's content
. Draft - anti-criteria based on common failure modes
. Apply the -question test (from eval-guide.md) to each criterion:
...
. Drop any criterion that fails a question
. Present to user for approval/modification before starting loop

---

 Prerequisites (completed before the loop starts)

The Algorithm's OBSERVE → THINK → PLAN → BUILD phases plus Phase  have already:
. Defined the optimization target (metric or eval criteria, measurement method, mutable files/target)
. Generated an initial hypothesis queue
. Set up sandbox (git branch for metric mode, directory copy for eval mode)
. Established baseline measurement
. Created results.tsv with header and baseline row
. Defined guard rail ISC criteria (assertions that must hold across all experiments)
. If eval mode: approved eval criteria and test inputs
...
LOOP:
  . ASSESS
     ...
     - Check current score against target:
       ...
       - Eval mode: current pass rate vs target pass rate (default: %)
     - If either limit reached → exit loop → proceed to Phase 
  ...
  . DECIDE
     - IMPROVED (score better AND guard rails pass):
       → Compute MAD confidence (see Confidence Gating section) if ≥ scores in session
       ...
       → If confidence is red (<.×), tag the row `marginal` in results.tsv and add a line to ` Dead Ends` if the improvement does not survive a re-run
     ...
     - WORSE or VIOLATION or CRASH:
       → Metric mode: git reset --hard HEAD~
       → Eval mode: restore sandbox from last known good state
       → Append reverted approach to ` Dead Ends` ledger with -line reasoning
       → Log: "REVERTED: {reason}"
  ...
  . LOG
     ...
     - Update ISA  Experiments table (keep last  rows + summary)

  . ADAPT
     - + consecutive reverts of same type → switch experiment category
     - Score plateaued → enter Plateau Protocol
     - EVAL MODE ISC ROTATION:
       - If pass rate > % but output quality feels stale → refine/tighten eval criteria
       - If one criterion passes % for + experiments → it's saturated:
...
. If metric_extract is specified — run it against stdout. Must produce one number.
. If output is JSON — look for metric_name as a key, extract its value.
. Fallback — extract the last numeric value from stdout.
. Failure — log "parse_error", revert experiment, continue loop.
...
- `confidence = |delta| / max(MAD, ε)` where `ε = e-` prevents division by zero on degenerate sessions

Gate activation: only compute `confidence` once the session has ≥  experiments (baseline +  attempts). Before that, MAD is statistically unstable; log `confidence = -` and defer entirely to direction + structural rationale.

Interpretation:

| Confidence | Signal | Action |
|------------|--------|--------|
| `≥ .×` | green — magnitude above session noise | DECIDE as normal |
| `. – .×` | yellow — above noise, marginal | keep if structural rationale holds; tag row `marginal` |
| `< .×` | red — within noise floor | recommend re-run or revert unless the change has independent structural justification |

Edge cases:
- Fewer than  experiments: `confidence = -`, skip gating entirely (see Gate activation above).
- MAD =  (all scores identical within measurement precision): the `ε` floor yields very high confidence for any non-zero delta. Log `confidence = ∞` and treat any change as signal; direction still governs keep/revert.
- Regression with high magnitude: `confidence` reads high because it's magnitude-only. This is correct — high-confidence regressions should revert emphatically, not pass. Never read `confidence ≥ .×` as "keep."
...
Pattern from pi-autoresearch (davebcn, MIT), itself inspired by Karpathy's original `autoresearch`.
...
. Construct judging prompt:
   ...
   ANSWER:
   ```
. Send to LLM via PAI Inference Tool (`bun TOOLS/Inference.ts fast`)
. Parse response: starts with "YES" → pass, starts with "NO" → fail
. Any other response → retry once, then count as fail
...
ON CRASH:
  ...
  . If trivial fix (typo, missing import) → fix and retry as same experiment
  . If fundamental → log as crash, move to next hypothesis
  . If + crashes in a row → pause, present status, ask user
```
...
When improvement stalls (< .% improvement over last  experiments):

Level  (after  stagnant experiments):
- Re-read all successful experiments, look for untried combinations
- Try combining  successful approaches that were tested independently
...
Level  (after  stagnant experiments):
- Pause and report: "Score plateaued at {value} after {N} experiments. Top improvements were: {list}. Options: () Continue with aggressive experiments, () Accept current result, () Expand scope, () Rotate eval criteria."
...
- Plateau Level  — user chooses to stop

On exit, proceed to Phase : RECOMMEND. Do NOT skip Phases -.
...
```
RECOMMEND:
  . Generate diff between original and optimized version:
  ...
  . Summarize results:
     ...
     - Time spent
  . List top - changes that drove the most improvement
  . List remaining failure patterns (what still doesn't work)
  . Present as recommendation via AskUserQuestion with options:
     ...
     - "Apply-Fanout": split kept experiments into independent merge-base branches (metric mode only)
       ...
       - Propose logical groupings to the user. Rules:
         - Preserve application order (group N before N+)
         - No two groups touch the same file (merge-base conflict) — merge overlapping groups
         - Flag cross-file dependencies ("group  calls API added in group  — review together")
       ...
       - Edge:  kept → skip Fanout (nothing to split);  kept → single branch, no grouping prompt
       - Pattern from pi-autoresearch's `autoresearch-finalize` skill (davebcn, MIT)
  . On accept → apply changes to real target
  . On any outcome → proceed to Phase 
```
...
```
EXTRACT LEARNINGS:
  . Distill what worked into structured learnings:
  ...
  . Write learnings to MEMORY/LEARNING/ as a learning signal:
...
experiment	commit	score	delta	delta_pct	confidence	status	type	description	timestamp
	baseline	.	.	.%	-	baseline	-	Initial measurement	--T::Z
	abcd	.	-.	-.%	.×	kept	elimination	Removed unused CSS	--T::Z
	defg	.	+.	+.%	.×	reverted	substitution	Replaced flexbox with grid	--T::Z
```

`confidence` is the MAD ratio `|delta|/MAD(all_scores)` — `-` until + experiments exist, `∞` when MAD=.

For eval mode, `score` is the pass rate (.-.), `commit` is "snapshot-N":

```
experiment	commit	score	delta	delta_pct	confidence	status	type	description	timestamp
	baseline	.	.	.%	-	baseline	-	Initial measurement	--T::Z
	snapshot-	.	+.	+.%	.×	kept	add_instruction	Added specificity requirement	--T::Z
	snapshot-	.	-.	-.%	.×	reverted	reorder_priority	Moved format spec to top	--T::Z
```
...
- `substitution`: flexbox → grid — .% regression, no structural gain
- `parameter_tune`: concurrency= →  — crashed on constrained hardware
- `algorithmic`: pre-compute lookup table — blew memory ceiling, no speed gain
```

Pattern from pi-autoresearch's living-doc `autoresearch.md` (davebcn, MIT).
...
```markdown
- [x] ISC-: Test suite passes after every kept change
- [x] ISC-: No type errors in mutable files
- [x] ISC-: Anti: No hardcoded values replacing computed values
```
...
```markdown
- [x] ISC-: Target produces non-empty output for all test inputs
- [x] ISC-: Target completes without errors for all test inputs
- [x] ISC-: Anti: No test input produces identical output to another
```
...
|---------------|----------------|-------|
| Phase : TARGET ANALYSIS | BUILD | Runs during BUILD, before loop |
| Loop steps - | EXECUTE | The loop replaces normal EXECUTE |
| Phase : RECOMMEND | VERIFY | Replaces normal VERIFY criteria checking |
| Phase : EXTRACT LEARNINGS | LEARN | Extends normal LEARN with optimization-specific insights |
