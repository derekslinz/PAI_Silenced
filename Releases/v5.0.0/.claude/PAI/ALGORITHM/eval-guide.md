...
Question : Would two independent judges agree?
The criterion must be specific enough that two different LLM instances (or two humans) would reach the same yes/no answer >% of the time. If reasonable judges would disagree, the criterion is too subjective.

- PASS: "Does the output contain at least  specific facts with cited sources?"
- FAIL: "Is the output high quality?" (too vague — judges will disagree)
- FAIL: "Is the writing style good?" (subjective, no anchor)

Question : Could the target game this without genuinely improving?
...
- FAIL: "Does the output contain more than  words?" (padding with filler passes this)
- FAIL: "Does the output mention all input keywords?" (keyword stuffing passes this)

Question : Does the user actually care about this?
...
- FAIL: "Does the output use exactly  bullet points?" (arbitrary formatting rule)
...
Count: - criteria per target. Fewer than  gives insufficient signal. More than  creates noise and slows the loop.
...
- "The output must NOT exceed  words for a summary task."
...
"Be concise" + "include comprehensive details" will create oscillation. Resolve conflicts before starting the loop by adding thresholds: "Under  words" + "covers at least  key aspects."
...
| `reword_ambiguity` | Clarify vague language | Change "be thorough" → "cover at least  distinct aspects" |
...
| `add_example` | Include a concrete example of desired output | Add a -line example showing ideal structure |
| `remove_over_optimization` | Strip instructions that cause rigidity | Remove "always use exactly  bullet points" |
| `simplify` | Reduce instruction count, merge redundant rules | Consolidate  similar formatting rules into  |
| `constrain_scope` | Narrow the target's focus | "Focus on the top  findings, not all findings" |
...
| `add__plus_rules` | Overwhelming — causes instruction-following degradation |
...
. Check for ceiling — if pass rate is >%, the current criteria may be saturated. Tighten criteria or add harder ones.
. Analyze failure patterns — which criteria are failing? Focus mutations on those specific areas.
. Try subtractive mutations — remove instructions that may be causing interference.
. Swap one criterion — replace the lowest-discrimination criterion with a harder one.
. Vary test inputs — the current inputs may not stress the right behaviors.
...
- `criteria_count` = number of eval criteria (-)
- `runs_per_experiment` = how many times the target is run per experiment (default: )

Example:  criteria,  runs,  passes out of  possible = % pass rate.
