...
```
 ...
 🗂️ Learning Router (v.. — replaces narrow Knowledge capture)
...
```
🗂️ LEARNING APPLIED:
 🗂️ [learning ]: <type> → <target path> [✓ written]
 🗂️ [learning ]: <type> → [⏸ surfaced to user, awaiting consent]
 🗂️ [learning ]: <type> → [SKIP — already encoded in <X>]
```
...
🖊️ STORY: [ -word bullets in Paul Graham simplicity format for what the problem was, what we did, how it went, and what if anything is next, each on a line preceded by - ]
