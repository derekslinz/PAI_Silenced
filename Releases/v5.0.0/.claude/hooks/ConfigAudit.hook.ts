...
async function main() {
  try {
    ...
    // Wired on Write/Edit/MultiEdit (no native ConfigChange event), so most
    // invocations touch files other than settings.json. Only append to the
    // audit log when settings.json actually changed — never spam "unchanged".
    if (changedKeys.length === 1 && (changedKeys[0] === 'unchanged')) {
      process.exit(0);
    }
