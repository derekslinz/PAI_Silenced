...
check_requirement "Server file exists" \
    "[ -f $PAI_DIR/VoiceServer/server.ts ]" \
    "Server file missing at \$PAI_DIR/VoiceServer/server.ts"
...
if [ -z "$PORT_CHECK" ]; then
    ...
    echo "   Stop the conflicting service or change PORT in ~/.claude/.env"
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
fi
...
# Check ~/.claude/.env file exists
if [ -f ~/.claude/.env ]; then
    echo -e "${GREEN}✓ ~/.claude/.env file exists${NC}"
    
    # Check for ElevenLabs API key
    if grep -q "ELEVENLABS_API_KEY=" ~/.claude/.env 2>/dev/null; then
        ...
        if [[ "$API_KEY" == "your_api_key_here" ]] || [[ -z "$API_KEY" ]]; then
            ...
            if grep -q "ELEVENLABS_VOICE_ID=" ~/.claude/.env 2>/dev/null; then
        ...
        check_warning "ElevenLabs API key configured" \
            "false" \
            "Add ELEVENLABS_API_KEY=your_key to ~/.claude/.env for AI voices"
    fi
else
    check_warning "~/.claude/.env file exists" \
        "false" \
        "Create ~/.claude/.env and add ELEVENLABS_API_KEY=your_key for AI voices"
fi
...
if [ -f ~/Library/LaunchAgents/com.paivoice.server.plist ]; then
    ...
    if launchctl list | grep -q "com.paivoice.server"; then
        STATUS_LINE=$(launchctl list | grep "com.paivoice.server")
