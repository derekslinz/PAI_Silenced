...
if curl -s -f http://localhost:8888/health > /dev/null 2>&1; then
    ...
    echo "Port: 8888"
    echo "---"
    echo "Stop Server | bash='$VOICE_DIR/stop.sh' terminal=false refresh=true"
    echo "Restart Server | bash='$VOICE_DIR/restart.sh' terminal=false refresh=true"
else
    ...
    echo "---"
    echo "Start Server | bash='$VOICE_DIR/start.sh' terminal=false refresh=true"
fi

echo "---"
echo "Check Status | bash='$VOICE_DIR/status.sh' terminal=true"
...
echo "---"
echo "Open Voice Server Folder | bash='open $VOICE_DIR'"
echo "Uninstall | bash='$VOICE_DIR/uninstall.sh' terminal=true"