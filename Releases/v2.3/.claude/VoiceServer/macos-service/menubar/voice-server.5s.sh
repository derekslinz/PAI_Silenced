...
if curl -s "${SERVER_URL}/health" > /dev/null 2>&1; then
    ...
    if launchctl list | grep -q "${SERVICE_NAME}"; then
        ...
        if [ "$PID" != "-" ]; then
            ...
            echo "Service: Loaded but stopped | color=orange"
        fi
    else
        echo "Running manually (not as service) | color=orange"
    fi
    ...
    echo "Stop Server | bash='${VOICE_SERVER_DIR}/macos-service/voice-server-ctl.sh' param1=stop terminal=false refresh=true"
    ...
    if [ -f ~/Library/LaunchAgents/${SERVICE_NAME}.plist ]; then
        ...
        echo "---"
        echo "►Start Server | bash='${VOICE_SERVER_DIR}/macos-service/voice-server-ctl.sh' param1=start terminal=false refresh=true"
    else
