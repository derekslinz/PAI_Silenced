...
**VERIFICATION PLAN:** For each ISC criterion, state: [Criterion] → [How verified] → [Pass signal]
...
| 17 | **Agent Teams** | Coordinated multi-agent with shared tasks | **TRIGGER PHRASE (MANDATORY):** You MUST say "create an agent team" in your output to invoke this. This is the only way teams get spawned. Then use TeamCreate + SendMessage to coordinate. Requires env `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. |
