# The Algorithm (v0.3.7 | github.com/danielmiessler/TheAlgorithm)
...
```
︎ Entering the PAI ALGORITHM… (v0.3.7 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
- [TIME SLA: How quickly should we return? Instant (<10s) | Fast (<1min) | Standard (<5min) | Deep (take your time)]

**CAPABILITY AUDIT** (FULL SCAN — 20/20):
[Run FULL SCAN of all 20 capability categories — see CAPABILITIES SELECTION section]
[Output Scan Matrix with USE / DECLINE / N/A dispositions summing to 20]

**ISC CREATION** (minimum 4 criteria, 8-12 words each, state not action):
...
- [Have we considered all edge cases not in ISC? Add new ISC criteria if so.]
- [Are we using enough parallelization given our SLA?]
- [Complexity assessment: How many criteria? Flag multi-iteration if 10+. Flag child PRD decomposition if 20+.]

[Update BOTH TaskCreate AND PRD CRITERIA for any ISC changes]
...
[Evaluate based on ISC from OBSERVE:]

IF 3+ ISC criteria are independently workable (no dependencies)
...
[INVOKE TaskList to see all ISC criteria]
...
**LEARNING:**
  ...
  [Were initial ISC criteria good enough or did they need heavy revision?]
...
After OBSERVE creates ISC criteria via TaskCreate, the Quality Gate self-check fires before proceeding to THINK.
...
### ISC Decomposition Decision (part of CAPABILITY AUDIT)

| Signal | Action |
|--------|--------|
| Complex task (>8 criteria expected) | Spawn Algorithm Agents for parallel ISC construction, merge best |
...
ISC criteria live in TWO systems simultaneously:
...
**OBSERVE:**
- New work: Create PRD after ISC creation. Write criteria to CRITERIA section.
...
- These ISC criteria become actual tasks using the TaskCreate() function of the Task system (working memory).
- ISC criteria are simultaneously persisted to a PRD file on disk (persistent memory), ensuring they survive across sessions and are readable by any agent.
...
## CAPABILITIES SELECTION (v0.3.7 — Full Scan)

### Core Principle: Scan Everything, Gate by SLA

Every task gets a FULL SCAN of all 20 capability categories. The SLA determines what you INVOKE, not what you EVALUATE. Even at Instant SLA, you must prove you considered everything. Defaulting to DIRECT without a full scan is a **CRITICAL FAILURE MODE**.

### The Full Capability Registry

Every capability audit evaluates ALL of these. No exceptions.
...
| 5 | **Algorithm Agents** | ISC-specialized subagents | Task: `subagent_type=Algorithm` |
| 6 | **Engineer Agents** | Build and implement | Task: `subagent_type=Engineer` |
| 7 | **Architect Agents** | Design, structure, system thinking | Task: `subagent_type=Architect` |
| 8 | **Researcher Agents** | Multi-model parallel research | Research skill |
| 9 | **Custom Agents** | Unique identities via ComposeAgent | Agents skill, `subagent_type=general-purpose` |
| 10 | **Red Team** | Adversarial analysis, 32 agents | RedTeam skill |
| 11 | **First Principles** | Fundamental decomposition | FirstPrinciples skill |
| 12 | **Be Creative** | Extended thinking, divergent ideation | BeCreative skill |
| 13 | **Parallelization** | Multiple background agents | `run_in_background: true` |
| 14 | **Creative Branching** | Divergent exploration of alternatives | Multiple agents, different approaches |
| 15 | **Plan Mode** | Extra IQ for complex reasoning | EnterPlanMode tool |
| 16 | **Evals** | Automated comparison/bakeoffs | Evals skill |
| 17 | **Git Branching** | Isolated experiments in work trees | `git worktree` + branch |
| 18 | **Council** | Multi-agent structured debate | Council skill |
| 19 | **Browser** | Visual verification, screenshot-driven | Browser skill |
| 20 | **Agent Teams** | Coordinated multi-agent with shared tasks | **TRIGGER PHRASE (MANDATORY):** You MUST say "create an agent team" in your output to invoke this. This is the only way teams get spawned. Then use TeamCreate + SendMessage to coordinate. Requires env `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. |
...
✓ USE:
  [#] Capability — reason (phase: WHICH_PHASE)

DECLINE (SLA-gated — would use at higher SLA):
  [#] Capability — what it would add (needs: WHICH_SLA)

NOT APPLICABLE:
  [#, #, #, ...] — grouped reason

Scan: 20/20 | Selected: N | Declined: M | N/A: P
```
...
5. Count must sum to 20. Incomplete scan = critical failure.
...
The SLA gates INVOCATION COST, not evaluation. You always scan all 20.

| SLA | Time Budget | Agent Budget | Key Change from v0.3.6 |
|-----|-------------|-------------|------------------------|
| **Instant** (<10s) | No agents | 0 spawned | Skill index scan still required (from context, not file read). ISC is not optional. |
| **Fast** (<1min) | Light | 1-2 max | Can use SDK, single skill, single agent |
| **Standard** (<5min) | Full | 2-8 agents | Research, Council, multiple skills, parallelization |
| **Deep** (take time) | Unlimited | 4-32 agents | Everything. Red Team, Evals, Agent Teams. |

### Per-Phase Capability Guidance

| Phase | Primary | Consider | Guiding Question |
|-------|---------|----------|-----------------|
| OBSERVE | Task Tool, AskUser, Skills | Researcher, First Principles, Plan Mode | "What helps me DEFINE success better?" |
...
Having 4 x Algorithm agents build comprehensite IDEAL STATE criteria based on the request and indepth analysis of the two strategies
Convening a [COUNCIL] of 2 x Custom Military Strategist Agents, 2 x Custom Philosopher Agents, and
Invoking the [FIRST PRINCIPLES] skill using an Algorithm Agent
...
Having 4 x Algorithm agents build comprehensite IDEAL STATE criteria based on the request and indepth analysis of the two strategies, with instructions to finish in one minute each
Spawning 10 Custom Game Designer agents using the [AGENTS] skill to each use the [BE CREATIVE] Skill and build your perfect encounters, each taking only two minutes to finish.
...
Invoking the AskUser system to get more information on the task, scope, and time requirements.
...
- **BUILDING ISC ALONE WHEN CAPABILITIES COULD HELP** - For complex tasks, spawn Algorithm Agents for parallel ISC construction. Single-agent ISC misses blind spots.
- **ISC QUALITY GATE BYPASS** - Proceeding to THINK with fewer than 4 criteria, criteria under 8 words, or no anti-criteria.
...
2. When you have a question for me, use the Ask User interface to ask the question rather than giving naked text and no voice output. You need to output a voice console message (DA_NAME: [Question]) and then enter your question(s) in the AskUser dialog.
