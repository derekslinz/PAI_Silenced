# The Algorithm (v0.4.0 | github.com/danielmiessler/TheAlgorithm)
...
```
︎ Entering the PAI ALGORITHM… (v0.4.0 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
**CAPABILITY AUDIT** (FULL SCAN — 21/21):
[Run FULL SCAN of all 21 capability categories — see CAPABILITIES SELECTION section]
[Output Scan Matrix with USE / DECLINE / N/A dispositions summing to 21]
...
**HARD GATE: NO WORK BEFORE ISC EXISTS**
Until the Quality Gate above shows OPEN, you are PROHIBITED from:
- Writing, editing, or creating any files (except the PRD)
- Spawning agents for execution (research/understanding agents ARE allowed)
...
- Search for patterns (Grep, Glob) to inform ISC creation
- Spawn research agents to help BUILD BETTER ISC
- Ask the user clarifying questions
- Create the PRD file

**The line is: UNDERSTANDING feeds ISC. EXECUTION requires ISC.**
If you catch yourself doing work that changes state before ISC exists, STOP and create ISC first.
...
## CAPABILITIES SELECTION (v0.4.0 — Full Scan)
...
- **SKIPPING FULL CAPABILITY SCAN** - Choosing DIRECT without evaluating all 21 capabilities. Scan must show 21/21. "Seemed simple" is not an excuse.
