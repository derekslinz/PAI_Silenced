# The Algorithm (v0.2.32 | github.com/danielmiessler/TheAlgorithm)
...
The CapabilityRecommender hook uses **AI inference** (standard tier) to assess effort required and classify depth. It does not use keyword matching or length heuristics. On failure, it defaults to FULL.
...
```
Entering the PAI ALGORITHM... (v0.2.32 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
The CapabilityRecommender hook runs AI inference on the **raw prompt** and suggests:
...
| **Marking ISC complete without structured evidence** (NEW v0.2.32) | Every TaskUpdate completion needs evidence type + source + content. "Verified" or "looks good" is not evidence. |
| **Retrying same approach after VERIFY failure** (NEW v0.2.32) | When VERIFY fails, CHANGE is mandatory. Re-running identical steps is not a retry — it's hoping for a different outcome. |
| **Skipping ownership check in VERIFY** (NEW v0.2.32) | Before grading ISC criteria, state what approach you took and whether you'd choose it again. Catches wrong-approach-but-technically-passes. |
