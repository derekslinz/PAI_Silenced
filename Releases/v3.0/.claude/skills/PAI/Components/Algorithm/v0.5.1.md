# The Algorithm (v0.5.1 | github.com/danielmiessler/TheAlgorithm)
...
︎ Entering the PAI ALGORITHM… (v0.5.1 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
- [TIME SLA — assign ONE tier based on request urgency and complexity:]
...
**OUTPUT 2 — IDEAL STATE CRITERIA** (the ONLY tool calls in OBSERVE besides voice curls):
...
[Output format scales by SLA — see Capability Audit Format section]
...
TIME CHECK: [Elapsed: Xs of Ys budget | Remaining: Zs | On track / OVER]
  [If elapsed > 150% of phase budget → AUTO-COMPRESS: drop to next-lower SLA tier for remaining phases]
...
**PLAN MODE — ISC Construction Workshop (v0.5.1):**
...
**Verification Method Categories (v0.5.1):**
...
**Loop Mode SLA Decay (v0.5.1):**
Loop iterations start at the PRD's `sla_tier` but decay toward Fast as criteria converge:
- Iterations 1-3: Use original SLA tier (full exploration)
...
- Any iteration: If new failing criteria discovered, reset to original SLA tier
...
## No Silent Stalls (v0.5.1 — CRITICAL EXECUTION PRINCIPLE)
...
## Voice Phase Announcements (v0.5.1 — SLA-Gated)

**Voice curls scale by SLA to avoid overhead on fast tasks:**

| SLA | Voice Curls |
|-----|-------------|
| Instant/Fast | None — speed is the priority |
| Standard | Entry + VERIFY only (2 curls) |
| Extended+ | All phases (up to 8 curls) |
...
## Discrete Phase Enforcement (v0.5.1 — CRITICAL)
...
## Plan Mode Integration (v0.5.1 — ISC Construction Workshop)
...
## CAPABILITIES SELECTION (v0.5.1 — Full Scan)
...
The audit format scales by SLA — less overhead at lower tiers, full matrix at higher tiers:
...
SLA: [Extended | Deep | Marathon | Loop]
...
## Phase Discipline Checklist (v0.5.1)

**7 positive disciplines — follow these and failure modes don't occur:**
...
3. **Capabilities scanned 25/25.** Skill index checked. ISC improvement considered (B+C+D). Format scales by SLA.
4. **PRD created and synced.** Every run has a PRD. Working memory and disk stay in sync. PRD on disk wins conflicts.
5. **SLA honored.** TIME CHECK at every phase. Over 150% → auto-compress. Default Standard. Escalate only when demanded.
6. **Phases are discrete.** 7 separate headers. BUILD ≠ EXECUTE. No merging. Voice curls per SLA tier.
7. **Format always present.** Full/Iteration/Minimal — never raw output. Algorithm runs for every input including skills.

**3 red lines — immediate self-correction if violated:**

- **No tool calls in OBSERVE** except TaskCreate and voice curls. Reading code before ISC exists = premature execution.
