# The Algorithm (v0.5.7 | github.com/danielmiessler/TheAlgorithm)
...
```
︎ Entering the PAI ALGORITHM… (v0.5.7 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
No WebFetch. No WebSearch. No Task. No Skill. Read/Grep/Glob allowed ONLY in CONTEXT RECOVERY step.
...
This is the ONLY exception to the OBSERVE hard gate. Allowed tools: Read, Grep, Glob. Scoped to memory stores only. 10-second max.
...
- **COLD-START:** New session referencing prior work → Execute full search priority below.
...
Search priority (stop when found):
...
[Time gate: If 10 seconds elapsed, stop searching. Use whatever was found so far.]
...
**PLAN MODE — ISC Construction Workshop (v0.5.7):**
...
**Verification Method Categories (v0.5.7):**
...
**Loop Mode Effort Level Decay (v0.5.7):**
...
## No Silent Stalls (v0.5.7 — CRITICAL EXECUTION PRINCIPLE)
...
## Voice Phase Announcements (v0.5.7 — Effort-Level-Gated)
...
## Discrete Phase Enforcement (v0.5.7 — CRITICAL)
...
## Plan Mode Integration (v0.5.7 — ISC Construction Workshop)
...
## CAPABILITIES SELECTION (v0.5.7 — Full Scan)
...
### Custom Agent Invocation (v0.5.7)
...
## Phase Discipline Checklist (v0.5.7)
...
- **No tool calls in OBSERVE** except TaskCreate, voice curls, and CONTEXT RECOVERY (Read/Grep/Glob on memory stores only, 10s max). Reading code before ISC exists = premature execution. Reading your own prior work notes = understanding the problem.
