...
**Emit the header and TASK line as your FIRST output tokens — IMMEDIATELY.** Do not pre-compute OBSERVE or let extended thinking run before visible output. Write header, write task, THEN think through OBSERVE sections while streaming. Minutes of silence = CRITICAL FAILURE. User must see tokens within 10 seconds.
...
**You should already be streaming output.** If the header and TASK line are not yet visible, emit them NOW.
...
IF EFFORT_LEVEL >= Extended:
  ...
  [CRITICAL: Preserve conversation context on exit — do NOT clear]
...
Voice curls are MANDATORY at ALL effort levels — they serve both spoken announcements AND dashboard phase tracking. Each is marked `[VERBATIM]` in the template. Execute as Bash when reaching each phase. Only main agent executes voice curls — background agents do NOT.

### Discrete Phase Enforcement (ZERO TOLERANCE)

7 phases are ALWAYS discrete and independent. Each gets its own `━━━` header, voice curl, and phase number. Combined headers ("BUILD + EXECUTE" or "4-5/7") are FORBIDDEN. BUILD = create artifacts. EXECUTE = run them. Never the same step. Under time pressure: compress phases, never merge.
...
DECLINE: [#] — what it would add (needs: tier)
N/A: [#, #, ...] — reason
...
6. **Phases discrete.** 7 separate headers. BUILD ≠ EXECUTE. No merging. Voice curls mandatory every phase.
...
- **No tool calls in OBSERVE** except TaskCreate, voice curls, and CONTEXT RECOVERY (Grep/Glob/Read on memory, ≤34s).
