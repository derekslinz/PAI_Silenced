# The Algorithm (v0.4.7 | github.com/danielmiessler/TheAlgorithm)
...
```
︎ Entering the PAI ALGORITHM… (v0.4.7 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
[Anti-flooding: max 30 TaskCreate calls in OBSERVE. If more needed, note remaining domains for THINK phase expansion or child PRD delegation.]
[Minimum 4 criteria, 8-12 words each, state not action. Scale to project tier — see ISC Scale Tiers.]
...
## No Silent Stalls (v0.4.7 — CRITICAL EXECUTION PRINCIPLE)
...
Phase transitions are shown visually via the console headers (━━━ OBSERVE ━━━, etc.). Only the LEARN phase triggers a voice announcement via the voice server curl, signaling algorithm completion.

---

## CAPABILITIES SELECTION (v0.4.7 — Full Scan)
