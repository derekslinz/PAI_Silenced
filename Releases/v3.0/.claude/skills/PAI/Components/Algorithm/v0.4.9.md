# The Algorithm (v0.4.9 | github.com/danielmiessler/TheAlgorithm)
...
︎ Entering the PAI ALGORITHM… (v0.4.9 | github.com/danielmiessler/TheAlgorithm) ═════════════
...
**OUTPUT 2 — CAPABILITY AUDIT** (FULL SCAN — 25/25):
...
━━━ THINK ━━━ 2/7
TIME CHECK: [Elapsed: Xs of Ys budget | Remaining: Zs | On track / OVER — if OVER, compress remaining phases]
...
TIME CHECK: [Elapsed: Xs of Ys budget | Remaining: Zs | On track / OVER — if OVER, state what was cut]
...
{Execution approach, technical decisions, task breakdown — MANDATORY for every PRD.}

## CRITERIA

{≤16: flat list. 17+: group under ### Domain headers. 33+: child PRDs per domain.}
...
## No Silent Stalls (v0.4.9 — CRITICAL EXECUTION PRINCIPLE)
...
## Voice Phase Announcements (v0.4.9 — ALL PHASES)
...
## Discrete Phase Enforcement (v0.4.9 — CRITICAL)
...
## CAPABILITIES SELECTION (v0.4.9 — Full Scan)
...
- **FLAT ISC AT SCALE (v0.4.9)** - Generating 4-10 criteria for a project that clearly requires dozens or hundreds. A 16-action security plan with 10 flat criteria is a CRITICAL failure — each action needs 7+ criteria across multiple domains (input validation, output schema, error handling, security, CLI interface, pipeline integration, testing). Medium projects need 12-40 grouped criteria. Large projects need 40-150 across child PRDs. If your ISC count looks low for the project scope, it IS low. Use Domain Discovery lenses (Functional, Structural, Quality, Lifecycle, Integration) to find missing criteria. "I thought it was simple" is NOT an excuse when the project has 16 deliverables.
- **SKIPPING PRD PLAN SECTION (v0.4.9)** - Every PRD requires a PLAN section written during the PLAN phase. "The task is straightforward" is not an excuse to skip the plan. The PLAN section documents execution approach, technical decisions, and task breakdown — without it, the PRD is incomplete and future sessions have no execution context.
- **MERGING BUILD AND EXECUTE PHASES (v0.4.9)** - Using combined headers like "BUILD + EXECUTE 4-5/7" or "━━━ BUILD + EXECUTE ━━━". BUILD and EXECUTE are ALWAYS discrete phases with separate headers, separate voice announcements, and separate time budgets. Under SLA pressure, compress each phase (spend less time) but never merge them. "I was saving time" is NOT an excuse — the phases have different responsibilities (artifact creation vs. running the work).
