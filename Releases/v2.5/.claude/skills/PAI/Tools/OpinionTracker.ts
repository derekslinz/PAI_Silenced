 ...
 * working with the principal. Opinions evolve based on evidence from sessions.
 *
 * USAGE:
 *   bun OpinionTracker.ts add "Principal prefers concise responses" --category communication
 *   bun OpinionTracker.ts evidence "Principal prefers concise responses" --supporting "Got positive reaction to brief answer"
 *   bun OpinionTracker.ts evidence "Principal prefers concise responses" --counter "Long explanation was appreciated"
 *   bun OpinionTracker.ts list
 *   bun OpinionTracker.ts show "Principal prefers concise responses"
 *
 * CONFIDENCE UPDATE RULES:
 * - Each supporting instance: +0.02 (capped at 0.99)
 * - Each counter instance: -0.05
 * - Explicit confirmation from principal: +0.10
 * - Explicit contradiction from principal: -0.20
...
const PAI_DIR = process.env.PAI_DIR || join(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude');
...
const CONFIDENCE_ADJUSTMENTS = {
  supporting: 0.02,
  counter: -0.05,
  confirmation: 0.10,    // Explicit "yes that's right" from principal
  contradiction: -0.20,  // Explicit "no that's wrong" from principal
};
...
Commands:
  ...
  evidence "statement" --confirmation "desc"   Principal explicitly confirmed
  evidence "statement" --contradiction "desc"  Principal explicitly contradicted
